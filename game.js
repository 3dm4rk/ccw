...
function applyDamage(defender, dmg, opts = {}) {
  ...
  // ✅ Zukinimato Form 7 passive: convert taken damage into healing + ATK buffs
  if (defender && defender.id === "zukinimato" && defender.zukiUltimatePassive && actualTaken > 0) {
    ...
    if (buff > 0) {
      ...
      const healed = heal(defender, buff);
...
function playerAttack() {
  ...
  const doOneAttack = (prefixMsg = "") => {
    ...
    applyDamage(e, dmg, { source: "attack", damageType: zukiTrue ? "true" : "physical", attacker: p, attackerName: p.name });
