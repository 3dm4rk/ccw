...
const BASE_CARDS = [
  {
    ...
    skillDesc: "Apply Stun (1 turn) and Seal (2 turns: enemy cannot use skill), then deal 3 TRUE damage. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      foe.stun2Rounds = Math.max(Number(foe.stun2Rounds || 0) || 0, 1);
      foe.silenced = Math.max(Number(foe.silenced || 0) || 0, 2);
      applyDamage(foe, 3, { silent: true, source: "skill", damageType: "true", attackerName: me.name });

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} enforces Galactic Arrest! Stun (1), Seal (2), 3 TRUE dmg.` };
    }
  },
    {
    id: "luckyCat",
    ...
    skillDesc: "Roll: 1-40 (+3 HP, +2 Armor) • 41-80 (remove enemy armor + heal 3) • 81-100 (12 TRUE dmg, +6 HP, +4 Armor, reduce your cooldown by 1). (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };
      ...
      if (roll <= 40) {
        const healed = heal(me, 3);
        const gained = gainShield(me, 2);
        msg = `🎲 Fortune Twist (Common ${roll})! ${healed ? `+${healed} HP` : "healing blocked"}, +${gained} Armor.`;
        me.cooldown = 3;
      } else if (roll <= 80) {
        const removed = Math.max(0, Number(foe.shield || 0) || 0);
        foe.shield = 0;
        foe.def = 0;
        const healed = heal(me, 3);
        msg = `🎲 Fortune Twist (Rare ${roll})! Removed ${removed} enemy Armor, ${healed ? `+${healed} HP` : "healing blocked"}.`;
        me.cooldown = 3;
      } else {
        applyDamage(foe, 12, { silent: true, source: "skill", damageType: "true", attackerName: me.name });
        const healed = heal(me, 6);
        const gained = gainShield(me, 4);
        me.cooldown = 3;
        // Jackpot reduces cooldown by 1
        me.cooldown = Math.max(0, Number(me.cooldown || 0) - 1);
        msg = `🎰 Fortune Twist (JACKPOT ${roll})! 12 TRUE dmg, ${healed ? `+${healed} HP` : "healing blocked"}, +${gained} Armor, cooldown -1.`;
      }
  ...
  // NEW 7 pack
  {
    ...
    skillDesc: "Instantly attack twice. Each hit ignores 1 Armor. If below 50% HP, gain Dodge (1 turn): next enemy attack misses. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      const belowHalf = (Number(me.hp || 0) || 0) < Math.ceil((Number(me.maxHp || 1) || 1) * 0.5);
      if (belowHalf) me.dodge = Math.max(Number(me.dodge || 0) || 0, 1);

      // Two hits, each with 1 armor pierce
      const hits = [];
      for (let i = 0; i < 2; i++) {
        if (foe && Number(foe.hp) > 0) {
          ...
          applyDamage(foe, dmg, { silent: true, source: "attack", damageType: "physical", attacker: me, attackerName: me.name });
          hits.push(dmg);
        }
      }
      ...
      me.cooldown = 3;
      updateUI();
      const dodgeNote = belowHalf ? " Dodge (1 turn)" : "";
      return { ok: true, msg: `${me.name} goes Overdrive! 2x attacks (${hits.join("+") || 0}) with armor pierce.${dodgeNote}` };
    }
  },
    {
    id: "patrickDestroyer",
    ...
    skillDesc: "Strike 3 times (2, 2, then 6 TRUE). If enemy drops under 25% HP, gain +3 ATK permanently. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      if (foe && Number(foe.hp) > 0) applyDamage(foe, 2, { silent: true, source: "skill", attackerName: me.name });
      if (foe && Number(foe.hp) > 0) applyDamage(foe, 2, { silent: true, source: "skill", attackerName: me.name });
      if (foe && Number(foe.hp) > 0) applyDamage(foe, 6, { silent: true, source: "skill", damageType: "true", attackerName: me.name });

      const thresh = Math.floor((Number(foe.maxHp || 1) || 1) * 0.25);
      if (foe && Number(foe.hp) > 0 && Number(foe.hp) <= thresh) {
        me.atk = Math.max(0, Number(me.atk || 0) || 0) + 3;
      }

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} runs Execution Protocol! 2 + 2 + 6 TRUE. ${me.atk >= 7 ? "(+3 ATK if threshold met)" : ""}` };
    }
  },
    {
    ...
    skillDesc: "Steal 50% of enemy Armor (rounded up) and deal damage equal to stolen Armor. If enemy has 0 Armor, steal 3 ATK instead. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      const enemyArmor = Math.max(0, Number(foe.shield || 0) || 0);
      if (enemyArmor > 0) {
        const stolen = Math.ceil(enemyArmor * 0.5);
        foe.shield = Math.max(0, enemyArmor - stolen);
        foe.def = foe.shield;
        const gained = gainShield(me, stolen);
        applyDamage(foe, stolen, { silent: true, source: "skill", attackerName: me.name });
        me.cooldown = 3;
        updateUI();
        return { ok: true, msg: `${me.name} pillages the grave! Stole ${stolen} Armor (+${gained}), and deals ${stolen} damage.` };
      }

      // No armor: steal ATK instead
      const stealAtk = 3;
      foe.atk = Math.max(0, Number(foe.atk || 0) - stealAtk);
      me.atk = Math.max(0, Number(me.atk || 0) + stealAtk);

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} finds no armor... steals ${stealAtk} ATK instead!` };
    }
  },
  {
  id: "tremo",
  ...
  skill: (me, foe) => {
    ...
    skillDesc: "Gain +4 Armor, heal +3 HP, and gain Sanctuary (2 turns): first SKILL hit is reduced by 50% and reflected as TRUE damage. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };
      ...
      me.sanctuaryTurns = Math.max(Number(me.sanctuaryTurns || 0) || 0, 2);
      me.sanctuaryUsed = 0;

      me.cooldown = 3;
      updateUI();

      const healNote = healed > 0 ? `+${healed} HP` : `healing blocked`;
      return { ok: true, msg: `${me.name} swears a Divine Oath! +${gained} Armor, ${healNote}, Sanctuary (2 turns).` };
    }
  },
    {
    id: "baltrio",
    ...
    skillDesc: "Deal 4 damage, remove ALL enemy Armor, and apply Void Anchor (2 turns): enemy cannot gain Armor and takes 2 TRUE damage each turn. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      applyDamage(foe, 4, { silent: true, source: "skill", attackerName: me.name });
      ...
      foe.poisonRounds = Math.max(Number(foe.poisonRounds || 0) || 0, 2);
      foe.poisonPct = 0;
      foe.poisonFlat = Math.max(Number(foe.poisonFlat || 0) || 0, 2);

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} pins reality with Void Anchor! 4 dmg, removed ${removed} Armor, no Armor gain + 2 TRUE/turn (2 turns).` };
    }
  },
];
...
nebulaGunslinger: {
    ...
    skillDesc: "Mark enemy (3 turns): they take +3 damage from all hits; when they use a skill, they take 4 recoil damage. Then fire 3 shots of 2 damage. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      foe.markedTurns = Math.max(Number(foe.markedTurns || 0) || 0, 3);
      foe.markBonus = Math.max(Number(foe.markBonus || 0) || 0, 3);
      foe.recoilOnSkill = Math.max(Number(foe.recoilOnSkill || 0) || 0, 4);
      ...
      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} tags a Bounty Mark! Mark (3 turns) + 3 shots.` };
    }
  },
solarPriestessSeraph: {
    ...
    skillDesc: "Heal +6 HP and gain +4 Armor. If healing would overheal, convert the excess into +2 Max HP permanently (stackable). (CD 2)",
    skill: (me) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      const beforeHp = Number(me.hp || 0) || 0;
      const beforeMax = Number(me.maxHp || 1) || 1;

      const healed = heal(me, 6);
      const gained = gainShield(me, 4);

      // Overheal conversion
      const missing = Math.max(0, beforeMax - beforeHp);
      const extra = Math.max(0, 6 - missing);
      if (extra > 0) {
        me.maxHp = beforeMax + 2;
        me.hp = Math.min(me.maxHp, (Number(me.hp || 0) || 0) + 2);
      }

      me.cooldown = 3;
      updateUI();
      const healNote = healed > 0 ? `+${healed} HP` : "healing blocked";
      const scaleNote = extra > 0 ? " (+2 Max HP)" : "";
      return { ok: true, msg: `${me.name} ascends! ${healNote}${scaleNote}, +${gained} Armor.` };
    }
  },
nebulaBladeDancer: {
    ...
    skillDesc: "Gain Parry (1 turn): next attack or skill that hits you is reflected 100% as normal damage. Then counterattack for 6 damage. (CD 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      me.parry = Math.max(Number(me.parry || 0) || 0, 1);
      me.parryCounterDamage = 6;

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} enters Starstep Parry! (Reflect next hit + counter)` };
    }
  },
...
novaEmpress: {
  ...
  skillDesc: "Deal 8 damage. If enemy Armor > 0, deal +6 bonus damage. If enemy dies, heal to full and gain +5 Max HP permanently (stackable). (CD 3)",
  skill: (me, foe) => {
    if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

    const hadArmor = (Number(foe.shield || 0) || 0) > 0;
    applyDamage(foe, 8, { silent: true, source: "skill", attackerName: me.name });
    if (foe && Number(foe.hp) > 0 && hadArmor) {
      applyDamage(foe, 6, { silent: true, source: "skill", attackerName: me.name });
    }
    ...
    me.cooldown = 3;
    updateUI();
    return { ok: true, msg: `${me.name} triggers Royal Detonation!${hadArmor ? " Bonus vs armor!" : ""}` };
  }
},
voidSamurai: {
  id: "voidSamurai",
  ...
  skillDesc: "Gain +4 Armor, deal 6 damage, and enter Counter Stance (2 turns): when enemy attacks, they take 3 TRUE damage back; when enemy uses skill, you gain +2 Armor. (CD 2)",
  skill: (me, foe) => {
    if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };
    ...
    me.counterStanceTurns = Math.max(Number(me.counterStanceTurns || 0) || 0, 2);
    me.counterTrue = 3;
    me.counterGainArmorOnSkill = 2;

    me.cooldown = 3;
    updateUI();
    return { ok: true, msg: `${me.name} uses Void Riposte! +${gained} Armor, 6 dmg, Counter Stance (2 turns).` };
  }
},
astroWitch: {
  id: "astroWitch",
  ...
  skillDesc: "Deal 10 TRUE damage. Apply Constellation Curse (2 turns): enemy loses -4 ATK each turn and their skill cooldown is +1 longer. (CD 3)",
  skill: (me, foe) => {
    if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

    applyDamage(foe, 10, { silent: true, source: "skill", damageType: "true", attackerName: me.name });

    foe.debuffRounds = Math.max(Number(foe.debuffRounds || 0) || 0, 2);
    foe.debuffAtk = Math.max(Number(foe.debuffAtk || 0) || 0, 4);

    foe.skillCdPenaltyTurns = Math.max(Number(foe.skillCdPenaltyTurns || 0) || 0, 2);

    me.cooldown = 3;
    updateUI();
    return { ok: true, msg: `${me.name} casts Astral Rewrite! 10 TRUE dmg + Constellation Curse (2 turns).` };
  }
},
  ...
  lore: "A wandering blade-spirit who turns pain into power—when Siyokou strikes true, even fate bleeds for you.",
    ...
    skillDesc: "50% chance to do Critical damage (15–75). Heal based on generated number (stackable, can go above current life) and gain +5 DEF. (CD 1)",
    skill: (me, foe) => {
...
diablo: {
    ...
    skillDesc: "Deal 10 damage, gain +5 Armor, and apply Hell Brand (3 turns): enemy healing -50%; whenever enemy heals, they take 5 TRUE damage. (CD 3)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      applyDamage(foe, 10, { silent: true, source: "skill", attackerName: me.name });
      const gained = gainShield(me, 5);

      foe.healReductionTurns = Math.max(Number(foe.healReductionTurns || 0) || 0, 3);
      foe.healReductionPct = 0.5;
      foe.hellBrandTurns = Math.max(Number(foe.hellBrandTurns || 0) || 0, 3);
      foe.hellBrandSourceUid = me.uid;

      me.cooldown = 3;
      updateUI();
      return { ok: true, msg: `${me.name} ignites Soul Furnace! 10 dmg, +${gained} Armor, Hell Brand (3 turns).` };
    }
  },
...
const POTIONS = [
  ...
  },
  {
    ...
    name: "Potion of Galactic Wanderer",
    price: 500000,
...
// =========================
// 🆚 PVP CARD POOL (ALL CARDS UNLOCKED)
// - PvP should ignore ownership / mission locks / secret gating.
// - Includes enemy-only and secret cards for fair PvP testing.
// =========================
function getAllCardsPvP() {
  const defs = (typeof UNLOCKABLE_CARD_DEFS !== "undefined" && UNLOCKABLE_CARD_DEFS) ? UNLOCKABLE_CARD_DEFS : {};
  const allUnlockables = Object.keys(defs).map((id) => defs[id]).filter(Boolean);

  const map = new Map();
  [...(BASE_CARDS || []), ...(allUnlockables || [])].forEach((c) => {
...
// =========================
// 🎮 PvP (2-player, 1 device)
// - Works like Battle mode, but the "enemy" is controlled by Player 2.
// - All cards are treated as MAX level.
// - Optional rules: potions, relics, score target.
// =========================
let __pvpLastSetup = null;

function getMaxedStats(cardDef) {
  const atk0 = Number(cardDef && cardDef.atk) || 0;
  const def0 = Number(cardDef && cardDef.def) || 0;
  const hp0  = Number(cardDef && cardDef.hp)  || 0;
  // Matches CARD_UPGRADE_MAX_LEVEL=5 bonuses in getUpgradedStats()
  return { atk: atk0 + 20, def: def0 + 20, hp: hp0 + 20, level: 5 };
}

function cloneCardPvP(card) {
  const st = getMaxedStats(card);
  return {
    uid: (__CARD_UID++),
    id: card.id,
    name: card.name,
...
// 🧠 Draft Mode (PvP variant: 5-card draft pool)
function closePvPDraftModal() {
  const el = document.getElementById('pvpDraftOverlay');
  if (el) el.remove();
}

function openPvPDraftModal() {
  if (document.getElementById('pvpDraftOverlay')) return;

  const overlay = document.createElement('div');
  overlay.className = 'modalOverlay';
  overlay.id = 'pvpDraftOverlay';

  const box = document.createElement('div');
  box.className = 'modalBox';

  const relicOptions = (RELICS || []).map(r => `<option value="${String(r.id)}">${r.name}</option>`).join('');

  const defaults = {
    potionsEnabled: true,
    enableGalacticWanderer: true,
    enableGodsUltimate: true,
    potionLoadoutEnabled: false,
    relicsEnabled: true,
    relicP1: String(state.equippedRelicId || ''),
    relicP2: '',
    pointsToWin: 1,
    draftPoolSize: 5
  };
  ...
  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🧠 Draft Mode (PvP)</div>
        <div class="modalPill">Each player drafts 1 from ${defaults.draftPoolSize}</div>
      </div>
      <button class="btn btnSoft" id="btnPvPDraftClose">✖</button>
    </div>

    <div class="modalBody">
      <p class="modalText">
        Draft rules: Player 1 sees ${defaults.draftPoolSize} random cards and picks 1. Then Player 2 sees ${defaults.draftPoolSize} random cards from the remaining pool (Player 1’s pick removed). Battle starts like normal PvP.
      </p>

      <div class="panelCard" style="margin-top:12px;">
        <div class="panelTitle"><h3>Rules</h3></div>

        <div style="margin-top:10px;display:grid;gap:10px;">
          <div class="modalStat">
            <div class="modalStatLabel">Potions</div>
            <div style="margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;">
              <button class="btn btnGhost" id="pvpDraftTogglePotions">Enabled</button>
              <span class="pill">Applies to both players</span>
            </div>
            <div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
              <button class="btn btnGhost" id="pvpDraftTogglePotionLoadout">Off</button>
              <span class="pill">Each player selects up to 2 (no duplicates)</span>
            </div>
            <div style="margin-top:10px;display:grid;gap:8px;">
              <button class="btn btnSoft" id="pvpDraftToggleGalactic">🧪 Potion of Galactic Wanderer: ON</button>
              <button class="btn btnSoft" id="pvpDraftToggleGods">🧪 God's Ultimate: ON</button>
            </div>
          </div>

          <div class="modalStat">
            <div class="modalStatLabel">Relics (custom)</div>
            <div style="margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
              <button class="btn btnGhost" id="pvpDraftToggleRelics">Enabled</button>
              <span class="pill">Each player chooses 1</span>
            </div>
            <div style="margin-top:10px;display:grid;gap:10px;">
              <label class="muted" style="font-weight:900;">Player 1 Relic</label>
              <select id="pvpDraftRelicP1" class="redeemCodeInput pvpRelicSelect" style="text-transform:none;letter-spacing:.2px;">
                <option value="">(None)</option>
                ${relicOptions}
              </select>
              <label class="muted" style="font-weight:900;">Player 2 Relic</label>
              <select id="pvpDraftRelicP2" class="redeemCodeInput pvpRelicSelect" style="text-transform:none;letter-spacing:.2px;">
                <option value="">(None)</option>
                ${relicOptions}
              </select>
            </div>
          ...
          <div class="modalStat">
            <div class="modalStatLabel">Score</div>
            <div class="muted" style="margin-top:6px;font-weight:900;">Points needed to win</div>
            <div style="margin-top:10px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
              <input id="pvpDraftPoints" type="number" min="1" value="1" class="redeemCodeInput" style="max-width:160px;text-transform:none;letter-spacing:.2px;" />
              <span class="pill">Default: 1</span>
            </div>
          </div>
        </div>
      </div>

      <div class="modalHint">Start Draft: Confirm rules → Player 1 drafts → Player 2 drafts → Battle starts.</div>
    </div>

    <div class="modalActions">
      <button class="btn btnGhost" id="btnPvPDraftCancel">Cancel</button>
      <button class="btn btnPrimary" id="btnPvPDraftConfirm">Start Draft</button>
    </div>
  `;
  ...
  function syncButtons() {
    const bPot = box.querySelector('#pvpDraftTogglePotions');
    const bLoad = box.querySelector('#pvpDraftTogglePotionLoadout');
    const bGal = box.querySelector('#pvpDraftToggleGalactic');
    const bGod = box.querySelector('#pvpDraftToggleGods');
    const bRel = box.querySelector('#pvpDraftToggleRelics');
    if (bPot) bPot.textContent = rules.potionsEnabled ? 'Enabled' : 'Disabled';
    if (bLoad) bLoad.textContent = (rules.potionLoadoutEnabled ? 'On' : 'Off');
    if (bGal) bGal.textContent = `🧪 Potion of Galactic Wanderer: ${rules.enableGalacticWanderer ? 'ON' : 'OFF'}`;
    if (bGod) bGod.textContent = `🧪 God's Ultimate: ${rules.enableGodsUltimate ? 'ON' : 'OFF'}`;
    if (bRel) bRel.textContent = rules.relicsEnabled ? 'Enabled' : 'Disabled';
    if (bGal) bGal.disabled = !rules.potionsEnabled;
    if (bGod) bGod.disabled = !rules.potionsEnabled;
    if (bLoad) bLoad.disabled = !rules.potionsEnabled;
    const s1 = box.querySelector('#pvpDraftRelicP1');
    const s2 = box.querySelector('#pvpDraftRelicP2');
    if (s1) s1.disabled = !rules.relicsEnabled;
    if (s2) s2.disabled = !rules.relicsEnabled;
  }

  const s1 = box.querySelector('#pvpDraftRelicP1');
  const s2 = box.querySelector('#pvpDraftRelicP2');
  if (s1) s1.value = rules.relicP1;
  if (s2) s2.value = rules.relicP2;

  syncButtons();

  const close = () => { try { playSfx('sfxClick', 0.25); } catch(e) {} closePvPDraftModal(); };
  const btnClose = box.querySelector('#btnPvPDraftClose');
  if (btnClose) btnClose.addEventListener('click', close);
  const btnCancel = box.querySelector('#btnPvPDraftCancel');
  if (btnCancel) btnCancel.addEventListener('click', close);
  overlay.addEventListener('click', (e) => { if (e.target === overlay) closePvPDraftModal(); });

  const bPot = box.querySelector('#pvpDraftTogglePotions');
  if (bPot) bPot.addEventListener('click', () => { rules.potionsEnabled = !rules.potionsEnabled; syncButtons(); });
  const bLoad = box.querySelector('#pvpDraftTogglePotionLoadout');
  if (bLoad) bLoad.addEventListener('click', () => { rules.potionLoadoutEnabled = !rules.potionLoadoutEnabled; syncButtons(); });
  const bGal = box.querySelector('#pvpDraftToggleGalactic');
  if (bGal) bGal.addEventListener('click', () => { rules.enableGalacticWanderer = !rules.enableGalacticWanderer; syncButtons(); });
  const bGod = box.querySelector('#pvpDraftToggleGods');
  if (bGod) bGod.addEventListener('click', () => { rules.enableGodsUltimate = !rules.enableGodsUltimate; syncButtons(); });
  const bRel = box.querySelector('#pvpDraftToggleRelics');
  if (bRel) bRel.addEventListener('click', () => { rules.relicsEnabled = !rules.relicsEnabled; syncButtons(); });

  if (s1) s1.addEventListener('change', () => { rules.relicP1 = s1.value; });
  if (s2) s2.addEventListener('change', () => { rules.relicP2 = s2.value; });

  const pts = box.querySelector('#pvpDraftPoints');
  if (pts) pts.addEventListener('change', () => {
    rules.pointsToWin = Math.max(1, Number(pts.value || 1) || 1);
    pts.value = String(rules.pointsToWin);
  });

  const btnConfirm = box.querySelector('#btnPvPDraftConfirm');
  if (btnConfirm) btnConfirm.addEventListener('click', () => {
    rules.pointsToWin = Math.max(1, Number((pts && pts.value) || 1) || 1);
    const enabledPotionIds = [];
    if (rules.potionsEnabled && rules.enableGalacticWanderer) enabledPotionIds.push('potionGalacticWanderer');
    if (rules.potionsEnabled && rules.enableGodsUltimate) enabledPotionIds.push('potionGodsUltimate');

    const payload = {
      potionsEnabled: !!rules.potionsEnabled,
      enabledPotionIds,
      potionLoadoutEnabled: !!rules.potionLoadoutEnabled,
      relicsEnabled: !!rules.relicsEnabled,
      relicP1: rules.relicsEnabled ? String(rules.relicP1 || '') : '',
      relicP2: rules.relicsEnabled ? String(rules.relicP2 || '') : '',
      pointsToWin: rules.pointsToWin,
      draftPoolSize: Math.max(2, Number(rules.draftPoolSize || 5) || 5)
    };

    closePvPDraftModal();
    openPvPDraftSelect(payload);
  });
}

function openPvPModal() {
  if (document.getElementById("pvpOverlay")) return;
  ...
  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🎮 PvP (2 Players • 1 Device)</div>
        <div class="modalPill">Max-level cards</div>
      </div>
      <button class="btn btnSoft" id="btnPvPClose">✖</button>
    </div>

    <div class="modalBody">
      <p class="modalText">Players take turns tapping buttons. Player 2 uses the buttons under the Enemy area.</p>
      ...
      <div class="panelCard" style="margin-top:12px;">
        ...
        <div style="margin-top:10px;display:grid;gap:10px;">
          <div class="modalStat">
            <div class="modalStatLabel">Potions</div>
            <div style="margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;">
              <button class="btn btnGhost" id="pvpTogglePotions">Enabled</button>
              <span class="pill">Applies to both players</span>
            </div>
            <div style="margin-top:10px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
              <button class="btn btnGhost" id="pvpTogglePotionLoadout">Off</button>
              <span class="pill">Each player selects up to 2 (no duplicates)</span>
            </div>
            <div style="margin-top:10px;display:grid;gap:8px;">
              <button class="btn btnSoft" id="pvpToggleGalactic">🧪 Potion of Galactic Wanderer: ON</button>
              <button class="btn btnSoft" id="pvpToggleGods">🧪 God's Ultimate: ON</button>
            </div>
          </div>

          <div class="modalStat">
            <div class="modalStatLabel">Relics (all unlocked)</div>
            <div style="margin-top:8px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
              <button class="btn btnGhost" id="pvpToggleRelics">Enabled</button>
              <span class="pill">Each player chooses 1</span>
            </div>
            <div style="margin-top:10px;display:grid;gap:10px;">
              <label class="muted" style="font-weight:900;">Player 1 Relic</label>
              <select id="pvpRelicP1" class="redeemCodeInput pvpRelicSelect" style="text-transform:none;letter-spacing:.2px;">
                <option value="">(None)</option>
                ${relicOptions}
              </select>
              <label class="muted" style="font-weight:900;">Player 2 Relic</label>
              <select id="pvpRelicP2" class="redeemCodeInput pvpRelicSelect" style="text-transform:none;letter-spacing:.2px;">
                <option value="">(None)</option>
                ${relicOptions}
              </select>
            </div>
          ...
          <div class="modalStat">
            ...
            <div class="muted" style="margin-top:6px;font-weight:900;">Points needed to win</div>
            <div style="margin-top:10px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
              <input id="pvpPoints" type="number" min="1" value="1" class="redeemCodeInput" style="max-width:160px;text-transform:none;letter-spacing:.2px;" />
              <span class="pill">Default: 1</span>
            </div>
          </div>
        </div>
      ...
      <div class="modalHint">Start Match Flow: Confirm rules → Player 1 picks card → Player 2 picks card → Battle starts.</div>
    </div>

    <div class="modalActions">
      <button class="btn btnGhost" id="btnPvPCancel">Cancel</button>
      <button class="btn btnPrimary" id="btnPvPConfirm">Confirm</button>
    </div>
  `;
  ...
  function syncButtons() {
    const bPot = box.querySelector('#pvpTogglePotions');
    const bLoad = box.querySelector('#pvpTogglePotionLoadout');
    const bGal = box.querySelector('#pvpToggleGalactic');
    const bGod = box.querySelector('#pvpToggleGods');
    const bRel = box.querySelector('#pvpToggleRelics');
    if (bPot) bPot.textContent = rules.potionsEnabled ? 'Enabled' : 'Disabled';
    if (bLoad) bLoad.textContent = (rules.potionLoadoutEnabled ? 'On' : 'Off');
    if (bGal) bGal.textContent = `🧪 Potion of Galactic Wanderer: ${rules.enableGalacticWanderer ? 'ON' : 'OFF'}`;
    if (bGod) bGod.textContent = `🧪 God's Ultimate: ${rules.enableGodsUltimate ? 'ON' : 'OFF'}`;
    ...
    if (bGod) bGod.disabled = !rules.potionsEnabled;
    if (bLoad) bLoad.disabled = !rules.potionsEnabled;
    const s1 = box.querySelector('#pvpRelicP1');
    const s2 = box.querySelector('#pvpRelicP2');
    if (s1) s1.disabled = !rules.relicsEnabled;
    if (s2) s2.disabled = !rules.relicsEnabled;
  }
  ...
  const close = () => { try { playSfx('sfxClick', 0.25); } catch(e) {} closePvPModal(); };

  const btnClose = box.querySelector('#btnPvPClose');
  if (btnClose) btnClose.addEventListener('click', close);
  const btnCancel = box.querySelector('#btnPvPCancel');
  if (btnCancel) btnCancel.addEventListener('click', close);
  overlay.addEventListener('click', (e) => { if (e.target === overlay) closePvPModal(); });
  ...
  const pts = box.querySelector('#pvpPoints');
  if (pts) pts.addEventListener('change', () => { rules.pointsToWin = Math.max(1, Number(pts.value || 1) || 1); pts.value = String(rules.pointsToWin); });

  const btnConfirm = box.querySelector('#btnPvPConfirm');
  if (btnConfirm) btnConfirm.addEventListener('click', () => {
    rules.pointsToWin = Math.max(1, Number((pts && pts.value) || 1) || 1);
    ...
    __pvpLastSetup = payload;
    closePvPModal();
    openPvPCardSelect(payload);
  });
}

function openPvPCardSelect(rules) {
  if (document.getElementById('pvpPickOverlay')) return;

  const overlay = document.createElement('div');
  overlay.className = 'modalOverlay';
  overlay.id = 'pvpPickOverlay';
  ...
  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🃏 PvP Card Select</div>
        <div class="modalPill" id="pvpPickPill">Player 1</div>
      </div>
      <button class="btn btnSoft" id="btnPvPPickClose">✖</button>
    </div>
    <div class="modalBody">
      <p class="modalText" id="pvpPickText">Player 1: choose your card (all cards unlocked • max level).</p>
      <div class="duelPickScroll" aria-label="Choose your fighter">
        <div id="pvpPickGrid" class="grid" style="margin-top:0;"></div>
      </div>
    </div>
    <div class="modalActions">
      <button class="btn btnGhost" id="btnPvPPickBack" disabled>Back</button>
      <button class="btn btnPrimary" id="btnPvPPickConfirm" disabled>Confirm Selected</button>
    </div>
  `;
  ...
  const grid = box.querySelector('#pvpPickGrid');
  const pill = box.querySelector('#pvpPickPill');
  const txt = box.querySelector('#pvpPickText');
  const btnBack = box.querySelector('#btnPvPPickBack');
  const btnOk = box.querySelector('#btnPvPPickConfirm');

  const all = (typeof getAllCardsPvP === 'function') ? (getAllCardsPvP() || [])
    : ((typeof getGalleryCards === 'function') ? (getGalleryCards() || [])
    : ((typeof getAllCards === 'function') ? (getAllCards() || []) : []));
  const cards = all.filter(Boolean).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));
  ...
  function render() {
    ...
    if (pill) pill.textContent = (step === 1 ? 'Player 1' : 'Player 2');
    if (txt) txt.textContent = (step === 1
      ? 'Player 1: choose your card (all cards unlocked • max level).'
      : 'Player 2: choose your card (all cards unlocked • max level).');

    if (btnBack) btnBack.disabled = (step === 1);

    cards.forEach((c) => {
      const up = getMaxedStats(c);
      const tile = document.createElement('div');
      tile.className = 'cardPick';
      tile.style.touchAction = 'pan-y';
      const safeName = String(c.name || 'Card');
      tile.innerHTML = `
        <img src="${c.img}" alt="${safeName}" onerror="this.onerror=null;this.src=window.__cardPlaceholder('${safeName.replace(/'/g, "\'")}')" />
        <div style="margin-top:10px;font-weight:1000;letter-spacing:.2px;">${safeName} <span class='pill' style='margin-left:6px;'>Lv${up.level}</span></div>
        <div class="muted" style="margin-top:6px;font-weight:900;">ATK ${up.atk} • DEF ${up.def} • HP ${up.hp}</div>
        <div class="muted" style="margin-top:6px;">${c.skillName}</div>
      `;

      let _downX=0,_downY=0,_moved=false;
      tile.addEventListener('pointerdown', (e)=>{ _moved=false; _downX=e.clientX; _downY=e.clientY; try{tile.setPointerCapture(e.pointerId);}catch(_){} }, {passive:true});
      tile.addEventListener('pointermove', (e)=>{
        const dx=abs(e.clientX-_downX), dy=abs(e.clientY-_downY);
        if (dx>10||dy>10) _moved=true;
      }, {passive:true});
      tile.addEventListener('pointerup', ()=>{
        ...
        grid.querySelectorAll('.cardPick').forEach(x=>x.style.outline='');
        tile.style.outline = '2px solid rgba(125,255,178,.55)';
  ...
  const close = () => { const el=document.getElementById('pvpPickOverlay'); if(el) el.remove(); };
  const closeBtn = box.querySelector('#btnPvPPickClose');
  if (closeBtn) closeBtn.addEventListener('click', () => { playSfx('sfxClick',0.25); close(); });
  overlay.addEventListener('click', (e)=>{ if(e.target===overlay) close(); });
  ...
  if (btnOk) btnOk.addEventListener('click', ()=>{
    ...
    if (step === 1) {
      ...
      p2 = selectedId;
      close();
      // Optional potion loadouts: each player selects up to 2 unique potions
      if (rules && rules.potionsEnabled && rules.potionLoadoutEnabled && Array.isArray(rules.enabledPotionIds) && rules.enabledPotionIds.length) {
        openPvPPotionLoadoutSelect(rules, p1, p2);
      } else {
        startPvPMatch(p1, p2, rules);
      }
    }
  });
}

// 🧠 Draft Mode: each player drafts 1 card from a small random pool.
function openPvPDraftSelect(rules) {
  if (document.getElementById('pvpDraftPickOverlay')) return;

  const overlay = document.createElement('div');
  overlay.className = 'modalOverlay';
  overlay.id = 'pvpDraftPickOverlay';

  const box = document.createElement('div');
  box.className = 'modalBox';

  const poolSize = Math.max(2, Number((rules && rules.draftPoolSize) || 5) || 5);

  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🧠 Draft Pool</div>
        <div class="modalPill" id="pvpDraftPickPill">Player 1</div>
      </div>
      <button class="btn btnSoft" id="btnPvPDraftPickClose">✖</button>
    </div>
    <div class="modalBody">
      <p class="modalText" id="pvpDraftPickText">Player 1: pick 1 card from your draft pool (${poolSize} random cards).</p>
      <div class="duelPickScroll" aria-label="Choose your draft card">
        <div id="pvpDraftPickGrid" class="grid" style="margin-top:0;"></div>
      </div>
      <div class="modalHint" id="pvpDraftHint" style="margin-top:12px;">Tip: Draft pools are random each match to reduce repetitive “best pick” play.</div>
    </div>
    <div class="modalActions single">
      <button class="btn btnPrimary" id="btnPvPDraftPickConfirm" disabled>Confirm Selected</button>
    </div>
  `;
  ...
  const grid = box.querySelector('#pvpDraftPickGrid');
  const pill = box.querySelector('#pvpDraftPickPill');
  const txt = box.querySelector('#pvpDraftPickText');
  const btnOk = box.querySelector('#btnPvPDraftPickConfirm');

  const all = (typeof getAllCardsPvP === 'function') ? (getAllCardsPvP() || [])
    : ((typeof getGalleryCards === 'function') ? (getGalleryCards() || [])
    : ((typeof getAllCards === 'function') ? (getAllCards() || []) : []));
  const basePool = all.filter(Boolean);
  ...
  function renderPool(forPlayer, bannedId) {
    const pool = (bannedId ? basePool.filter(c => String(c.id) !== String(bannedId)) : basePool);
    currentPool = sample(pool, poolSize);
    render(forPlayer);
  }

  function render(forPlayer) {
    if (!grid) return;
    grid.innerHTML = '';
    selectedId = null;
    if (btnOk) btnOk.disabled = true;

    if (pill) pill.textContent = (forPlayer === 1 ? 'Player 1' : 'Player 2');
    if (txt) txt.textContent = (forPlayer === 1
      ? `Player 1: pick 1 card from your draft pool (${poolSize} random cards).`
      : `Player 2: pick 1 card from your draft pool (${poolSize} random cards). (Player 1’s card removed)`);
  ...
  const close = () => { const el = document.getElementById('pvpDraftPickOverlay'); if (el) el.remove(); };
  const closeBtn = box.querySelector('#btnPvPDraftPickClose');
  if (closeBtn) closeBtn.addEventListener('click', () => { playSfx('sfxClick', 0.25); close(); });
  overlay.addEventListener('click', (e)=>{ if (e.target === overlay) close(); });
  ...
  if (btnOk) btnOk.addEventListener('click', () => {
    ...
    if (step === 1) {
      p1 = selectedId;
      step = 2;
      // Player 2 pool: remove Player 1’s chosen card
      renderPool(2, p1);
    } else {
      p2 = selectedId;
      close();
...
function openPvPPotionLoadoutSelect(rules, p1CardId, p2CardId) {
  if (document.getElementById('pvpPotionPickOverlay')) return;

  const enabledIds = new Set((rules.enabledPotionIds || []).map(String));
  const options = (POTIONS || []).filter(p => enabledIds.has(String(p.id)));

  const overlay = document.createElement('div');
  overlay.className = 'modalOverlay';
  overlay.id = 'pvpPotionPickOverlay';
  ...
  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🧪 PvP Potion Loadout</div>
        <div class="modalPill" id="pvpPotionPickPill">Player 1</div>
      </div>
      <button class="btn btnSoft" id="btnPvPPotionPickClose">✖</button>
    </div>
    <div class="modalBody">
      <p class="modalText" id="pvpPotionPickText">Player 1: choose up to 2 potions (no duplicates).</p>
      <div class="potionChoices" id="pvpPotionPickChoices"></div>
      <div class="muted" style="margin-top:10px;font-weight:900;" id="pvpPotionPickCount">Selected: 0 / 2</div>
    </div>
    <div class="modalActions">
      <button class="btn btnGhost" id="btnPvPPotionPickBack" disabled>Back</button>
      <button class="btn btnPrimary" id="btnPvPPotionPickConfirm">Confirm Selected</button>
    </div>
  `;
  ...
  const pill = box.querySelector('#pvpPotionPickPill');
  const txt = box.querySelector('#pvpPotionPickText');
  const list = box.querySelector('#pvpPotionPickChoices');
  const cnt = box.querySelector('#pvpPotionPickCount');
  const btnBack = box.querySelector('#btnPvPPotionPickBack');
  const btnOk = box.querySelector('#btnPvPPotionPickConfirm');
  ...
  function render() {
    // NOTE: tempSel is the live working selection for the current step.
    // Don't overwrite it on every re-render, otherwise clicks appear to "not work".
    // We only rehydrate it when switching steps (Back / Confirm).
    if (pill) pill.textContent = (step === 1 ? 'Player 1' : 'Player 2');
    if (txt) txt.textContent = (step === 1
      ? 'Player 1: choose up to 2 potions (no duplicates).'
      : 'Player 2: choose up to 2 potions (no duplicates).');
    if (btnBack) btnBack.disabled = (step === 1);
    if (cnt) cnt.textContent = `Selected: ${tempSel.size} / 2`;

    if (!list) return;
    list.innerHTML = '';

    (options || []).forEach((potion) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'potionChoice';
      const isOn = tempSel.has(String(potion.id));

      const left = document.createElement('div');
      left.className = 'potionChoiceLeft';
      left.innerHTML = `
        <div class="potionChoiceName">${potion.name}</div>
        <div class="potionChoiceDesc">${potion.desc || ''}</div>
      `;
      const right = document.createElement('div');
      right.className = 'potionChoiceRight';
      right.textContent = isOn ? 'Selected' : 'Select';

      btn.appendChild(left);
      btn.appendChild(right);

      if (isOn) {
        btn.style.outline = '2px solid rgba(125,255,178,.55)';
      }
      ...
      btn.addEventListener('click', () => {
        ...
        if (tempSel.has(id)) {
          ...
          if (tempSel.size >= 2) {
            try { showToast('Max 2 potions.', 'warn'); } catch(e) {}
            return;
          }
          tempSel.add(id);
        }
        // Persist the working selection so render() doesn't reset it.
        if (step === 1) p1Sel = Array.from(tempSel);
        else p2Sel = Array.from(tempSel);
        if (cnt) cnt.textContent = `Selected: ${tempSel.size} / 2`;
        render();
      });
  ...
  function close() {
    const el = document.getElementById('pvpPotionPickOverlay');
    if (el) el.remove();
  }

  const closeBtn = box.querySelector('#btnPvPPotionPickClose');
  if (closeBtn) closeBtn.addEventListener('click', () => { playSfx('sfxClick', 0.25); close(); });
  overlay.addEventListener('click', (e) => { if (e.target === overlay) close(); });
  ...
  if (btnBack) btnBack.addEventListener('click', () => {
    playSfx('sfxClick', 0.25);
    if (step === 2) {
      // save P2 current before going back
      p2Sel = Array.from(tempSel);
      step = 1;
      tempSel = new Set((p1Sel || []).map(String));
      render();
    }
  });
  ...
  if (btnOk) btnOk.addEventListener('click', () => {
    playSfx('sfxClick', 0.35);
    if (step === 1) {
      p1Sel = Array.from(tempSel);
      step = 2;
      tempSel = new Set((p2Sel || []).map(String));
      render();
      return;
    }
    p2Sel = Array.from(tempSel);
    ...
    const mergedRules = {
      ...(rules || {}),
      p1PotionIds: Array.from(new Set((p1Sel || []).map(String))).slice(0, 2),
      p2PotionIds: Array.from(new Set((p2Sel || []).map(String))).slice(0, 2)
    };
    // Remember full setup (including loadouts) for Rematch
    __pvpLastSetup = mergedRules;
    startPvPMatch(p1CardId, p2CardId, mergedRules);
  });

  render();
}

function startPvPFromLastSetup() {
  if (!__pvpLastSetup) throw new Error('No previous PvP setup');
  // Re-open pick flow with last rules
  openPvPCardSelect(__pvpLastSetup);
}

function startPvPMatch(p1CardId, p2CardId, rules) {
  // PvP should allow ANY card (including secret/enemy-only) regardless of unlock state.
  const pvpPool = (typeof getAllCardsPvP === 'function') ? (getAllCardsPvP() || []) : [];
  const pvpById = new Map((pvpPool || []).filter(Boolean).map((c) => [String(c.id), c]));
  const c1 = pvpById.get(String(p1CardId)) || findCardById(p1CardId);
  const c2 = pvpById.get(String(p2CardId)) || findCardById(p2CardId);
  if (!c1 || !c2) {
    try { showToast('PvP failed: missing card data.', 'warn'); } catch(e) {}
    return;
  }

  // Preserve user's equipped relic so PvP doesn't overwrite their account settings
  if (!state._pvpPrevEquipped) state._pvpPrevEquipped = String(state.equippedRelicId || '');

  state.pvpMode = true;
  state.pvpRules = rules || { pointsToWin: 1, potionsEnabled: true, enabledPotionIds: ['potionGalacticWanderer','potionGodsUltimate'], relicsEnabled: true, relicP1: '', relicP2: '' };

  // Normalize optional potion loadouts (each player can have up to 2 unique)
  if (state.pvpRules && state.pvpRules.potionLoadoutEnabled) {
    state.pvpRules.p1PotionIds = Array.from(new Set((state.pvpRules.p1PotionIds || []).map(String))).slice(0, 2);
    state.pvpRules.p2PotionIds = Array.from(new Set((state.pvpRules.p2PotionIds || []).map(String))).slice(0, 2);
  }
  state.pvpScore = { player: 0, enemy: 0 };
  state.pvpRelics = {
    player: String((rules && rules.relicP1) || ''),
    enemy: String((rules && rules.relicP2) || '')
  };

  // Relic context begins with Player 1
  state.equippedRelicId = state.pvpRelics.player || '';

  // Per-player potion cooldowns
  state.pvpPotionCd = { player: 0, enemy: 0 };

  // ✅ PvP potion anti-abuse: each player only gets 1 total potion use for the whole match.
  // Cooldowns still apply, but stock is limited so it can't be spammed.
  const potsEnabled = !!(state.pvpRules && state.pvpRules.potionsEnabled && (state.pvpRules.enabledPotionIds || []).length);
  state.pvpPotionStock = { player: potsEnabled ? 1 : 0, enemy: potsEnabled ? 1 : 0 };

  state.player = cloneCardPvP(c1);
  state.enemy = cloneCardPvP(c2);

  // Arena labels
  state.stageLabel = 'PvP';
  state.phase = 'battle';
  // ✅ Randomize who gets the first turn (was always Player 1)
  state.turn = (Math.random() < 0.5) ? 'player' : 'enemy';
  state.round = 1;

  // Clear log and enter game
  showView('game');
  resetCardVisuals();
  $('log').innerHTML = '';
  // Relic context should match the starting side
  state.equippedRelicId = (state.turn === 'player') ? (state.pvpRelics.player || '') : (state.pvpRelics.enemy || '');

  log(`🎮 PvP Match started! Player 1: ${state.player.name} vs Player 2: ${state.enemy.name}.`, 'warn');
  log(`🏁 First to ${Math.max(1, Number(state.pvpRules.pointsToWin||1)||1)} point(s) wins.`, 'warn');
  log(`🎲 First turn: ${state.turn === 'player' ? 'Player 1' : 'Player 2'}.`, 'warn');
  updateUI();
}

function pvpAttack(who) {
  if (!state.pvpMode) return;
  if (state.phase !== 'battle') return;
  if (state.turn !== (who === 'enemy' ? 'enemy' : 'player')) {
    try { showToast('⏳ Wait for your turn.', 'warn'); } catch(e) {}
    return;
  }

  // Swap relic context
  if (state.pvpRelics) state.equippedRelicId = (who === 'enemy') ? (state.pvpRelics.enemy || '') : (state.pvpRelics.player || '');

  const a = (who === 'enemy') ? state.enemy : state.player;
  const d = (who === 'enemy') ? state.player : state.enemy;
  const aLabel = (who === 'enemy') ? 'Player 2' : 'Player 1';

  state.lastAction = 'attack';

  if (a.frozen > 0) {
    log(`${aLabel} (${a.name}) is frozen and cannot act!`, 'warn');
    nextTurn();
    return;
  }
  if (a.stun2Rounds > 0) {
    log(`${aLabel} (${a.name}) is stunned and cannot act! (${a.stun2Rounds} rounds left)`, 'warn');
    nextTurn();
    return;
  }
  ...
  const dmg = dmgCalc(a);
  log(`${aLabel} (${a.name}) attacks for ${dmg} damage!`, who === 'enemy' ? 'bad' : 'good');

  spawnAttackFx(who === 'enemy' ? 'enemy' : 'player', who === 'enemy' ? 'player' : 'enemy');
  applyDamage(d, dmg, { source: 'attack', damageType: 'physical', attacker: a, attackerName: a.name });

  // Vampire Fang on attacker
  if (hasRelic('vampireFang')) {
    const healed = heal(a, 1);
    if (healed > 0) {
      log(`🪬 Vampire Fang heals ${aLabel} +1 HP.`, who === 'enemy' ? 'bad' : 'good');
      floatingDamage(who === 'enemy' ? 'enemy' : 'player', '+1', 'good');
    } else {
      log(`🪬 Vampire Fang tried to heal, but Reboot Seal blocked it!`, 'warn');
    }
  }

  // Chrono Rune: extra attack
  if (hasRelic('chronoRune') && d && Number(d.hp) > 0 && Math.random() < 0.20) {
    const dmg2 = dmgCalc(a);
    log(`⏳ Chrono Rune triggers! Extra attack for ${dmg2} damage!`, who === 'enemy' ? 'bad' : 'good');
    spawnAttackFx(who === 'enemy' ? 'enemy' : 'player', who === 'enemy' ? 'player' : 'enemy');
    applyDamage(d, dmg2, { source: 'attack', damageType: 'physical', attacker: a, attackerName: a.name });
    if (hasRelic('vampireFang')) {
      const healed2 = heal(a, 1);
      if (healed2 > 0) floatingDamage(who === 'enemy' ? 'enemy' : 'player', '+1', 'good');
    }
  }
...
function pvpSkill(who) {
  ...
  if (!isSkillReady(a)) {
    const cdTurns = Math.max(0, Number(a.cooldown || 0) || 0);
    const rtCd = (a.skillReadyAt && a.skillReadyAt > Date.now());
    const cdLabel = rtCd ? formatSkillCd(a) : `${cdTurns} turn(s)`;
    log(`${aLabel} (${a.name}) skill is on cooldown (${cdLabel}).`, 'warn');
    updateUI();
    return;
  }
  if (a.frozen > 0) {
  ...
  if (a.stun2Rounds > 0) {
    log(`${aLabel} (${a.name}) is stunned and cannot use a skill! (${a.stun2Rounds} rounds left)`, 'warn');
    nextTurn();
    return;
  }
  if (isSilenced(a)) { log(`${aLabel} (${a.name}) is Silenced and cannot use a skill!`, 'warn'); return; }

  const res = a.base.skill(a, d, state);
  if (!res || !res.ok) { log((res && res.msg) ? res.msg : 'Skill failed.', 'warn'); return; }

  playSfx('sfxSkill', 0.8);
  log(`✨ ${aLabel} uses skill: ${res.msg}`, who === 'enemy' ? 'bad' : 'good');

  updateUI();
  if (!checkWin()) nextTurn();
}

function pvpEndTurn(who) {
  if (!state.pvpMode) return;
  if (state.turn !== (who === 'enemy' ? 'enemy' : 'player')) {
    try { showToast('⏳ Wait for your turn.', 'warn'); } catch(e) {}
    return;
  ...
  playSfx('sfxEnd', 0.65);
  log(`${who === 'enemy' ? 'Player 2' : 'Player 1'} ended their turn.`, 'warn');
  nextTurn();
}

function pvpPotionReady(who) {
  if (!state.pvpPotionCd) state.pvpPotionCd = { player: 0, enemy: 0 };
  const key = (who === 'enemy') ? 'enemy' : 'player';
  return (Number(state.pvpPotionCd[key] || 0) || 0) <= 0;
}

function pvpPotionStockLeft(who) {
  if (!state.pvpPotionStock) state.pvpPotionStock = { player: 0, enemy: 0 };
  const key = (who === 'enemy') ? 'enemy' : 'player';
  return Math.max(0, Number(state.pvpPotionStock[key] || 0) || 0);
}

function pvpConsumePotionStock(who) {
  if (!state.pvpPotionStock) state.pvpPotionStock = { player: 0, enemy: 0 };
  const key = (who === 'enemy') ? 'enemy' : 'player';
  const left = Math.max(0, Number(state.pvpPotionStock[key] || 0) || 0);
  if (left <= 0) return false;
  state.pvpPotionStock[key] = left - 1;
  return true;
}
function pvpPotionCdLabel(who) {
  if (!state.pvpPotionCd) state.pvpPotionCd = { player: 0, enemy: 0 };
  const key = (who === 'enemy') ? 'enemy' : 'player';
  const t = Math.max(0, Number(state.pvpPotionCd[key] || 0) || 0);
  return t <= 0 ? 'Ready' : `${t} turn(s)`;
}
function pvpSetPotionCd(who, turns) {
  if (!state.pvpPotionCd) state.pvpPotionCd = { player: 0, enemy: 0 };
  const key = (who === 'enemy') ? 'enemy' : 'player';
  state.pvpPotionCd[key] = Math.max(0, Number(turns || 0) || 0);
}

function pvpUsePotionFlow(who) {
  if (!state.pvpMode || state.phase !== 'battle') return;
  if (state.turn !== (who === 'enemy' ? 'enemy' : 'player')) {
    try { showToast('⏳ Wait for your turn to use potions.', 'warn'); } catch(e) {}
    return;
  }

  const rules = state.pvpRules || {};
  if (!rules.potionsEnabled) {
    try { showToast('🧪 Potions are disabled for this match.', 'warn'); } catch(e) {}
    return;
  }

  // ✅ Hard limit: only 1 potion use per player for the whole PvP match.
  if (pvpPotionStockLeft(who) <= 0) {
    try { showToast('🧪 No potions left (PvP limit: 1).', 'warn'); } catch(e) {}
    return;
  }

  const enabled = new Set((rules.enabledPotionIds || []).map(String));
  let available = (POTIONS || []).filter((p) => enabled.has(String(p.id)));

  // If loadouts are enabled, only show that player's chosen potions (up to 2, no duplicates)
  if (rules.potionLoadoutEnabled) {
    const loadout = (who === 'enemy') ? (rules.p2PotionIds || []) : (rules.p1PotionIds || []);
    const loadSet = new Set((loadout || []).map(String));
    available = available.filter((p) => loadSet.has(String(p.id)));
    if (!available.length) {
      try { showToast('No potions selected for this player.', 'warn'); } catch(e) {}
      return;
    }
  }
  if (!available.length) {
    try { showToast('No potions enabled in rules.', 'warn'); } catch(e) {}
    return;
  }
  if (!pvpPotionReady(who)) {
    try { showToast(`Potion cooldown: ${pvpPotionCdLabel(who)}`, 'warn'); } catch(e) {}
    return;
  }

  openPvPPotionUseModal(available, who);
}

function closePvPPotionUseModal() {
  const el = document.getElementById('pvpPotionUseOverlay');
  if (el) el.remove();
}

function openPvPPotionUseModal(availablePotions, who) {
  closePvPPotionUseModal();

  const overlay = document.createElement('div');
  overlay.className = 'modalOverlay';
  overlay.id = 'pvpPotionUseOverlay';
  ...
  box.innerHTML = `
    <div class="modalHeader">
      <div>
        <div class="modalTitle">🧪 PvP Potion</div>
        <div class="muted" style="font-weight:900;margin-top:4px;">${who === 'enemy' ? 'Player 2' : 'Player 1'} • CD: ${pvpPotionCdLabel(who)} • Uses left: ${pvpPotionStockLeft(who)}</div>
      </div>
      <button class="btn btnSoft" id="btnPvPPotionClose">✖</button>
    </div>
    <div class="modalBody">
      <p class="modalText">Choose one potion. Cooldown applies only to the player who used it.</p>
      <div class="potionChoices" id="pvpPotionChoices"></div>
    </div>
    <div class="modalActions single">
      <button class="btn btnGhost" id="btnPvPPotionCancel">Cancel</button>
    </div>
  `;
  ...
  const list = box.querySelector('#pvpPotionChoices');
  (availablePotions || []).forEach((potion) => {
    ...
    btn.addEventListener('click', () => {
      ...
      // Swap relic context so shield-cap math matches this player
      if (state.pvpRelics) state.equippedRelicId = (who === 'enemy') ? (state.pvpRelics.enemy || '') : (state.pvpRelics.player || '');

      // ✅ Consume the player's single PvP potion stock
      if (!pvpConsumePotionStock(who)) {
        try { showToast('🧪 No potions left (PvP limit: 1).', 'warn'); } catch(e) {}
        closePvPPotionUseModal();
        updateUI();
        return;
      ...
      applyPotionEffect(potion, who === 'enemy' ? 'enemy' : 'player');
      pvpSetPotionCd(who, potion.cooldownTurns || 10);

      try { playSfx('sfxSkill', 0.55); } catch(e) {}
      try { log(`🧪 ${who === 'enemy' ? 'Player 2' : 'Player 1'} used ${potion.name}!`, who === 'enemy' ? 'bad' : 'good'); } catch(e) {}
      try { floatingDamage(who === 'enemy' ? 'enemy' : 'player', '🧪', 'good'); } catch(e) {}

      closePvPPotionUseModal();
      updateUI();

      // Potion counts as an action; end turn (matches Battle feel)
      if (!checkWin()) nextTurn();
    });
  ...
  const closeBtn = box.querySelector('#btnPvPPotionClose');
  if (closeBtn) closeBtn.addEventListener('click', () => { playSfx('sfxClick', 0.35); closePvPPotionUseModal(); });

  const cancel = box.querySelector('#btnPvPPotionCancel');
  if (cancel) cancel.addEventListener('click', () => { playSfx('sfxClick', 0.35); closePvPPotionUseModal(); });

  overlay.addEventListener('click', (e) => { if (e.target === overlay) closePvPPotionUseModal(); });
}
...
function cloneCard(card) {
  ...
  return {
    ...
    markBonus: 0,
    recoilOnSkill: 0,
    skillCdPenaltyTurns: 0,
    hellBrandTurns: 0,
    hellBrandSourceUid: 0,
    healReductionTurns: 0,
    healReductionPct: 0,
    counterStanceTurns: 0,
    counterTrue: 0,
    counterGainArmorOnSkill: 0,
...
function cardReviveFlip(target) {
  ...
  card.classList.remove("dieFlip", "isDead", "revivePulse", "reviveFlip");
...
function cardDieFlip(who) {
  ...
  card.classList.remove("dieFlip", "isDead");
  void card.offsetWidth;
  card.classList.add("dieFlip");

  // Keep a persistent "dead" look after the flip completes.
  // (Also guarantees the gray overlay stays even if the animation class is cleared later.)
  setTimeout(() => {
    try { card.classList.add("isDead"); } catch(e){}
  }, 600);
}
...
function applyDamage(defender, dmg, opts = {}) {
  ...
  // ✅ META: Mark (bonus damage)
  if (!opts.ignoreMark && (Number(defender.markedTurns || 0) || 0) > 0 && dmg > 0) {
    const bonus = Math.max(0, Number(defender.markBonus || 0) || 0);
    if (bonus > 0) dmg += bonus;
  }

  // ✅ META: Dodge (blocks next enemy ATTACK)
  if (!opts.noReactive && (Number(defender.dodge || 0) || 0) > 0 && source === "attack" && dmg > 0) {
    defender.dodge = Math.max(0, Number(defender.dodge || 0) - 1);
    const who = defender === state.player ? "player" : "enemy";
    floatingDamage(who, "MISS", "good");
    log(`💨 Dodge! ${defender.name} avoids the attack.`, "good");
    updateUI();
    return;
  }

  // ✅ META: Barrier (blocks 1 incoming SKILL completely)
  if (!opts.noReactive && (Number(defender.barrier || 0) || 0) > 0 && source === "skill" && dmg > 0) {
    defender.barrier = Math.max(0, Number(defender.barrier || 0) - 1);
    const who = defender === state.player ? "player" : "enemy";
    floatingDamage(who, "BARRIER", "good");
    log(`🛡️ Barrier blocks the skill!`, "good");
    updateUI();
    return;
  }

  // ✅ META: Parry (reflect next hit, then counterattack)
  if (!opts.noReactive && (Number(defender.parry || 0) || 0) > 0 && dmg > 0 && opts.attacker) {
    defender.parry = Math.max(0, Number(defender.parry || 0) - 1);
    const atk = opts.attacker;
    const reflect = Math.max(0, Number(dmg) || 0);
    log(`🗡️ Parry! ${defender.name} reflects the hit.`, "good");
    applyDamage(atk, reflect, { silent: true, source: "skill", damageType: "physical", attackerName: defender.name, noReactive: true, ignoreMark: true });
    const counter = Math.max(0, Number(defender.parryCounterDamage || 0) || 0);
    if (counter > 0 && Number(atk.hp) > 0) {
      applyDamage(atk, counter, { silent: true, source: "attack", damageType: "physical", attackerName: defender.name, noReactive: true, ignoreMark: true });
    }
    updateUI();
    return;
  }

  // ✅ META: Sanctuary (first SKILL hit reduced 50% and reflected as TRUE)
  let __sanctuaryPrevented = 0;
  if (!opts.noReactive && source === "skill" && dmg > 0 && (Number(defender.sanctuaryTurns || 0) || 0) > 0 && Number(defender.sanctuaryUsed || 0) !== 1 && opts.attacker) {
    const reduced = Math.ceil(Number(dmg) * 0.5);
    __sanctuaryPrevented = Math.max(0, Number(dmg) - reduced);
    dmg = reduced;
    defender.sanctuaryUsed = 1;
  }
  let absorbed = 0;
  let hpLoss = 0;
  ...
  // ✅ META: Sanctuary reflect
  if (__sanctuaryPrevented > 0 && opts.attacker && Number(opts.attacker.hp) > 0) {
    applyDamage(opts.attacker, __sanctuaryPrevented, { silent: true, source: "skill", damageType: "true", attackerName: defender.name, noReactive: true, ignoreMark: true });
    log(`✨ Sanctuary reflects ${__sanctuaryPrevented} TRUE damage back to ${opts.attacker.name}!`, "good");
    // End Sanctuary after it triggers once
    defender.sanctuaryTurns = 0;
  }

  // ✅ META: Counter Stance (Void Samurai)
  if (!opts.noReactive && (Number(defender.counterStanceTurns || 0) || 0) > 0 && source === "attack" && opts.attacker && Number(opts.attacker.hp) > 0) {
    const back = Math.max(0, Number(defender.counterTrue || 0) || 3);
    applyDamage(opts.attacker, back, { silent: true, source: "skill", damageType: "true", attackerName: defender.name, noReactive: true, ignoreMark: true });
    log(`🌀 Counter Stance! ${opts.attacker.name} takes ${back} TRUE damage back.`, "warn");
  }
...
function heal(fighter, amount) {
  ...
  // ✅ META: healing reduction (e.g., Hell Brand)
  const redPct = Math.max(0, Math.min(0.95, Number(fighter.healReductionPct || 0) || 0));
  if ((Number(fighter.healReductionTurns || 0) || 0) > 0 && redPct > 0) {
    amount = Math.max(0, Math.ceil(Number(amount) * (1 - redPct)));
  }
  ...
  // ✅ META: Hell Brand — healing punisher
  if (healed > 0 && (Number(fighter.hellBrandTurns || 0) || 0) > 0) {
    const srcUid = Number(fighter.hellBrandSourceUid || 0) || 0;
    const src = (state.player && state.player.uid === srcUid) ? state.player : ((state.enemy && state.enemy.uid === srcUid) ? state.enemy : null);
    if (src && Number(src.hp) > 0) {
      applyDamage(fighter, 5, { silent: true, source: "skill", damageType: "true", attackerName: src.name, noReactive: true });
      log(`🔥 Hell Brand punishes healing — ${fighter.name} takes 5 TRUE damage!`, "warn");
    }
  }
...
function tickStatuses(f) {
  ...
  // ✅ META timers
  if (f.sanctuaryTurns > 0) f.sanctuaryTurns -= 1;
  if (f.parry > 0) f.parry -= 1;
  if (f.dodge > 0) f.dodge -= 1;
  if (f.barrier > 0) f.barrier -= 1;
  if (f.markedTurns > 0) f.markedTurns -= 1;
  if (f.skillCdPenaltyTurns > 0) f.skillCdPenaltyTurns -= 1;
  if (f.recoilOnSkill > 0 && f.markedTurns <= 0) f.recoilOnSkill = 0;
  if (f.hellBrandTurns > 0) f.hellBrandTurns -= 1;
  if (f.healReductionTurns > 0) f.healReductionTurns -= 1;
  if (f.counterStanceTurns > 0) f.counterStanceTurns -= 1;

  if (f.sanctuaryTurns <= 0) f.sanctuaryUsed = 0;
  if (f.parry <= 0) f.parryCounterDamage = 0;
  if (f.markedTurns <= 0) { f.markBonus = 0; f.recoilOnSkill = 0; }
  if (f.skillCdPenaltyTurns <= 0) { /* no-op */ }
  if (f.hellBrandTurns <= 0) f.hellBrandSourceUid = 0;
  if (f.healReductionTurns <= 0) f.healReductionPct = 0;
  if (f.counterStanceTurns <= 0) { f.counterTrue = 0; f.counterGainArmorOnSkill = 0; }
...
function updateUI() {
  ...
  // ✅ Solo (Player vs AI) battle-only layout tweaks.
  // Must NOT affect PvP/Draft/Duel.
  try {
    const soloAiBattle = (state.phase === "battle") && !state.pvpMode && !state.duelMode;
    document.body.classList.toggle("soloAIBattle", !!soloAiBattle);
  } catch (err) {}

  // ✅ PvP/Draft battle-only styling hook (kept separate from solo layout tweaks).
  // This class is only used to scope button styling in PvP (including Draft mode).
  try {
    const pvpBattle = (state.phase === "battle") && !!state.pvpMode;
    document.body.classList.toggle("pvpBattle", !!pvpBattle);
  } catch (err) {}
  ...
  if (state.pvpMode) {
    const score = state.pvpScore ? `${state.pvpScore.player||0}-${state.pvpScore.enemy||0}` : "0-0";
    const target = Math.max(1, Number((state.pvpRules && state.pvpRules.pointsToWin) || 1) || 1);
    $("turnTag").textContent = `Turn: ${state.turn === "player" ? "Player 1" : "Player 2"}`;
    $("stageTag").textContent = `Mode: PvP`;
    const scEl = document.getElementById('pvpScoreCenter');
    if (scEl) {
      scEl.style.display = 'block';
      scEl.textContent = `Score: ${score} • First to ${target}`;
    }
    const ep = $("enemyPassiveTag"); if (ep) ep.textContent = "Enemy Passive: —";
    const ea = $("enemyAiTag"); if (ea) ea.textContent = "Enemy AI: PvP";
  } else {
    const scEl = document.getElementById('pvpScoreCenter');
  ...
  if (!state.pvpMode) $("stageTag").textContent = `Stage: ${stLabel}`;
  if (!state.pvpMode) {
  ...
  if (bs) {
    ...
    const cdTip = sil
      ...
      : (onCd
        ...
        : "Ready!");
    bs.title = `${p.base.skillName}: ${p.base.skillDesc}\n${cdTip}`;
  ...
  // 🎮 PvP: Enemy action dock mirrors player buttons
  const enemyDock = document.getElementById("enemyActionDock");
  // Keep enemy dock layout consistent with CSS flex-based action dock (prevents messy desktop layout)
  if (enemyDock) enemyDock.style.display = state.pvpMode ? "flex" : "none";

  if (state.pvpMode) {
    const enemyTurn = state.turn === "enemy" && state.phase === "battle";
    const p1Turn = state.turn === "player" && state.phase === "battle";

    // Player 1 potion (reuse existing btnPotion but with PvP rules/cd)
    const bp = document.getElementById("btnPotion");
    if (bp) {
      const rules = state.pvpRules || {};
      const enabled = rules.potionsEnabled && (rules.enabledPotionIds || []).length;
      const left = pvpPotionStockLeft("player");
      const canUse = enabled && left > 0 && pvpPotionReady("player");
      bp.disabled = !p1Turn || !enabled || left <= 0;

      if (!enabled) {
        bp.textContent = "🧪 Potions Disabled";
        bp.title = "Potions are disabled.";
      } else if (left <= 0) {
        bp.textContent = "🧪 Potion Used (0)";
        bp.title = "PvP potion limit: 1 total use";
      } else {
        bp.textContent = canUse ? "🧪 Use Potion (1)" : `🧪 Potion CD: ${pvpPotionCdLabel("player")} (1)`;
        bp.title = `Uses left: ${left}\nPotion cooldown: ${pvpPotionCdLabel("player")}`;
      }
    }

    // Enemy dock buttons
    const eAtk = document.getElementById("eBtnAttack");
    if (eAtk) eAtk.disabled = !enemyTurn;

    const eSkill = document.getElementById("eBtnSkill");
    if (eSkill) {
      const eF = state.enemy;
      const cdTurns = Math.max(0, Number(eF?.cooldown || 0) || 0);
      const rtCd = (eF?.skillReadyAt && eF.skillReadyAt > Date.now());
      const sil = isSilenced(eF);
      const onCd = rtCd || cdTurns > 0;
      const ready = enemyTurn && !sil && !onCd;
      eSkill.disabled = !ready;
      if (!enemyTurn) eSkill.textContent = "✨ Use Skill";
      else if (sil) eSkill.textContent = "🔇 Silenced";
      else if (onCd) eSkill.textContent = `⏳ Skill CD: ${rtCd ? formatSkillCd(eF) : `${cdTurns} turn(s)`}`;
      else eSkill.textContent = "✨ Use Skill";
    }

    const eEnd = document.getElementById("eBtnEnd");
    if (eEnd) eEnd.disabled = !enemyTurn;

    const ePot = document.getElementById("eBtnPotion");
    if (ePot) {
      const rules = state.pvpRules || {};
      const enabled = rules.potionsEnabled && (rules.enabledPotionIds || []).length;
      const left = pvpPotionStockLeft("enemy");
      const canUse = enabled && left > 0 && pvpPotionReady("enemy");
      ePot.disabled = !enemyTurn || !enabled || left <= 0;

      if (!enabled) {
        ePot.textContent = "🧪 Potions Disabled";
        ePot.title = "Potions are disabled.";
      } else if (left <= 0) {
        ePot.textContent = "🧪 Potion Used (0)";
        ePot.title = "PvP potion limit: 1 total use";
      } else {
        ePot.textContent = canUse ? "🧪 Use Potion (1)" : `🧪 Potion CD: ${pvpPotionCdLabel("enemy")} (1)`;
        ePot.title = `Uses left: ${left}\nPotion cooldown: ${pvpPotionCdLabel("enemy")}`;
      }
    }
  }
...
function checkWin() {
  ...
  // 🎮 PvP resolution (score-based)
  if (state.pvpMode) {
    if (p.hp <= 0 || e.hp <= 0) {
      // ✅ PvP: always show the death flip + full-card gray overlay.
      // Trigger this BEFORE updateUI() so rapid UI updates don't "skip" the animation.
      try {
        if (p.hp <= 0) cardDieFlip("player");
        if (e.hp <= 0) cardDieFlip("enemy");
      } catch(e) {}

      if (!state.pvpScore) state.pvpScore = { player: 0, enemy: 0 };
      const draw = (p.hp <= 0 && e.hp <= 0);
      const p1Wins = (e.hp <= 0 && p.hp > 0);
      const p2Wins = (p.hp <= 0 && e.hp > 0);

      if (draw) {
        log("🤝 Double KO! No point awarded.", "warn");
      } else if (p1Wins) {
        state.pvpScore.player += 1;
        log(`🏆 Point for Player 1! (${state.pvpScore.player}-${state.pvpScore.enemy})`, "good");
      } else if (p2Wins) {
        state.pvpScore.enemy += 1;
        log(`🏆 Point for Player 2! (${state.pvpScore.player}-${state.pvpScore.enemy})`, "bad");
      }

      state.phase = "over";
      updateUI();

      const target = Math.max(1, Number(state.pvpRules?.pointsToWin || 1) || 1);
      if (state.pvpScore.player >= target || state.pvpScore.enemy >= target) {
        const winner = state.pvpScore.player >= target ? "Player 1" : "Player 2";
        setTimeout(() => {
          state.modalAction = "pvp";
          const btnNext = $("btnNextEnemy");
          if (btnNext) btnNext.textContent = "🏠 Go Home";
          const btnPlay = $("btnPlayAgain");
          if (btnPlay) btnPlay.textContent = "🔁 Rematch";
          openModal({
            title: `🎮 PvP Winner: ${winner}`,
            text: `${winner} wins the match!`,
            stageLabel: `Score ${state.pvpScore.player}-${state.pvpScore.enemy}`,
            hint: `First to ${target} point(s).`,
            goldReward: 0,
            mode: "defeat"
          });
        }, 650);
      } else {
        // Next round: full restore both and continue (Player 1 starts)
        setTimeout(() => {
          // Clear death visuals for the new round.
          // (Keeps the flip/gray visible during the "over" phase timeout above.)
          resetCardVisuals();

          // ✅ PvP fix: restore shields using EACH player's relic context.
          // (Otherwise, if Player 2 ended the round, Player 1's shield cap could be computed with Player 2's relic.)
          const p1Relic = String(state.pvpRelics?.player || "");
          const p2Relic = String(state.pvpRelics?.enemy || "");

          p.hp = p.maxHp;
          state.equippedRelicId = p1Relic;
          p.shield = Math.min(getShieldCap(p), Number(p.shieldCap || p.shield || 0) || 0);

          e.hp = e.maxHp;
          state.equippedRelicId = p2Relic;
          e.shield = Math.min(getShieldCap(e), Number(e.shieldCap || e.shield || 0) || 0);

          p.cooldown = 0; e.cooldown = 0;
          p.skillReadyAt = 0; e.skillReadyAt = 0;
          p.frozen = 0; e.frozen = 0;
          p.stun2Rounds = 0; e.stun2Rounds = 0;
          p.silencedRounds = 0; e.silencedRounds = 0;
          p.noArmorGain = 0; e.noArmorGain = 0;
          p.rebootSeal = 0; e.rebootSeal = 0;
          state.phase = "battle";
          state.turn = "player";
          // Next round always starts with Player 1 relic active
          state.equippedRelicId = p1Relic;
          $("log").innerHTML = "";
          log("🔁 Next Round! Player 1 starts.", "warn");
          updateUI();

          // Keep previousRelic only for safety if something else depends on it later in the tick
          // (PvP will still restore the account relic on resetAll.)
          void prevRelic;
        }, 900);
      }
      return true;
...
function nextTurn() {
  ...
  // 🎮 PvP: no AI automation; each side clicks their own buttons.
  if (state.pvpMode) {
    ...
    // Per-player potion cooldown (turn-based)
    if (!state.pvpPotionCd) state.pvpPotionCd = { player: 0, enemy: 0 };
    const key = state.turn === "player" ? "player" : "enemy";
    state.pvpPotionCd[key] = Math.max(0, Number(state.pvpPotionCd[key] || 0) || 0);
    if (state.pvpPotionCd[key] > 0) state.pvpPotionCd[key] -= 1;

    // Count rounds when Player 1 starts
    if (state.turn === "player") state.round = Math.max(1, Number(state.round || 1) || 1) + 1;

    // Swap active relic context (so existing calculations use the right relic)
    if (state.pvpRelics) {
      state.equippedRelicId = state.turn === "player" ? (state.pvpRelics.player || "") : (state.pvpRelics.enemy || "");
    }
  ...
  if (action.type === "skill") {
    if (isSilenced(e)) {
      ...
      if (res && res.ok) {
        ...
        // ✅ META: Mark recoil on skill use
        if ((Number(e.recoilOnSkill || 0) || 0) > 0 && Number(e.hp) > 0) {
          applyDamage(e, Number(e.recoilOnSkill || 0) || 0, { silent: true, source: "skill", damageType: "true", attackerName: "Recoil", noReactive: true });
          log(`🎯 Mark recoil hits ${e.name} for ${e.recoilOnSkill} TRUE damage!`, "warn");
        }

        // ✅ META: Skill cooldown penalty
        if ((Number(e.skillCdPenaltyTurns || 0) || 0) > 0) {
          e.cooldown = Math.max(0, Number(e.cooldown || 0) + 1);
          log(`⏳ Constellation Curse extends ${e.name}'s cooldown!`, "warn");
        }

        // ✅ META: Void Samurai counter-stance reacts to enemy skill
        if (p && (Number(p.counterStanceTurns || 0) || 0) > 0 && (Number(p.counterGainArmorOnSkill || 0) || 0) > 0) {
          const g = gainShield(p, Number(p.counterGainArmorOnSkill || 0) || 0);
          if (g > 0) log(`🌀 Counter Stance reinforces ${p.name} (+${g} Armor).`, "warn");
        }
...
function playerSkill() {
  ...
  // ✅ META: Mark recoil on skill use
  if ((Number(p.recoilOnSkill || 0) || 0) > 0 && Number(p.hp) > 0) {
    applyDamage(p, Number(p.recoilOnSkill || 0) || 0, { silent: true, source: "skill", damageType: "true", attackerName: "Recoil", noReactive: true });
    log(`🎯 Mark recoil hits ${p.name} for ${p.recoilOnSkill} TRUE damage!`, "warn");
  }

  // ✅ META: Skill cooldown penalty
  if ((Number(p.skillCdPenaltyTurns || 0) || 0) > 0) {
    p.cooldown = Math.max(0, Number(p.cooldown || 0) + 1);
    log(`⏳ Constellation Curse extends ${p.name}'s cooldown!`, "warn");
  }

  // ✅ META: Void Samurai counter-stance reacts to enemy skill
  if (e && (Number(e.counterStanceTurns || 0) || 0) > 0 && (Number(e.counterGainArmorOnSkill || 0) || 0) > 0) {
    const g = gainShield(e, Number(e.counterGainArmorOnSkill || 0) || 0);
    if (g > 0) log(`🌀 Counter Stance reinforces ${e.name} (+${g} Armor).`, "warn");
  }
...
function applyPotionEffect(potion, who = "player") {
  const key = (who === "enemy") ? "enemy" : "player";
  const oppKey = key === "player" ? "enemy" : "player";
  const p = state[key];
  if (!p || !potion) return;
...
function resetAll() {
  ...
  // Close any secondary overlays (PvP / Duel / potion pickers)
  try { closePvPModal(); } catch (e) {}
  try {
    [
      "pvpPickOverlay",
      "pvpPotionUseOverlay",
      "pvpPotionPickOverlay",
      "duelOverlay",
      "potionUseOverlay"
    ].forEach((id) => {
      const el = document.getElementById(id);
  ...
  state.pvpRules = null;
  state.pvpScore = null;
  state.pvpRelics = null;
  state.pvpPotionCd = null;
  state.pvpPotionStock = null;

  // Restore equipped relic after PvP so it doesn't overwrite account settings
  if (state._pvpPrevEquipped != null) {
    state.equippedRelicId = String(state._pvpPrevEquipped || "");
    delete state._pvpPrevEquipped;
  }
...
// =========================
  ...
  if (state.modalAction === "pvp") {
    ...
    const btnPlay = $("btnPlayAgain");
    if (btnPlay) btnPlay.textContent = "🔄 Restart Run";
    // Restore equipped relic
    if (state._pvpPrevEquipped != null) state.equippedRelicId = String(state._pvpPrevEquipped || "");
    state.pvpMode = false;
    closeModal();
    resetAll();
    showView("home");
  ...
  if (state.modalAction === "pvp") {
    ...
    if (btnPlay) btnPlay.textContent = "🔄 Restart Run";
    closeModal();
    try { startPvPFromLastSetup(); } catch (e) { resetAll(); showView("home"); }
    return;
  }
  ...
  safeOn("btnSkill", () => {
    ...
    if (b) {
      // Don't fire cast FX if somehow clicked while disabled
    ...
    // 💥 tiny, satisfying shake on cast (especially nice on mobile)
  ...
  // 🎮 PvP: Player 2 action dock
  safeOn("eBtnAttack", () => { playSfx("sfxClick", 0.35); pvpAttack("enemy"); });
  safeOn("eBtnSkill", () => { playSfx("sfxClick", 0.35); pvpSkill("enemy"); });
  safeOn("eBtnEnd", () => { playSfx("sfxClick", 0.35); pvpEndTurn("enemy"); });
  safeOn("eBtnPotion", () => { playSfx("sfxClick", 0.35); pvpUsePotionFlow("enemy"); });
...
// Ability info (click the "i" icon to also print the description into the battle log)
  ...
  safeOn("btnPvP", () => { playSfx("sfxClick", 0.40); openPvPModal(); });
  safeOn("btnDraftMode", () => { playSfx("sfxClick", 0.40); openPvPDraftModal(); });
