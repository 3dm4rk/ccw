...
const GAME_VERSION = "battle-stability-v11";
...
// =========================
// 🛡️ LeiRality Passive: Shield Hero helpers
// =========================
...
,
  ...
  leiRality: {
    ...
    skillDesc: "Activation: Only usable at 30% HP or lower. 50% chance to gain +20 ATK (added to current). 100% chance to Stun the enemy for 1 turn. (cooldown 3)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      // Ensure originalMaxHp is set for the 30% threshold
      maybeInitLeiRality(me);
      const orig = Math.max(1, Number(me.originalMaxHp || me.maxHp || 1) || 1);
      const hpPct = (Number(me.hp || 0) || 0) / orig;

      if (hpPct > 0.30) {
        return { ok: false, msg: `Reality Requiem can only be used when HP is 30% or lower.` };
      }
      ...
      if (foe) foe.stunned = Math.max(1, Number(foe.stunned || 0) || 0);

      me.cooldown = 4; // cooldown 3
      return { ok: true, msg: `${me.name} uses Reality Requiem!${gained ? ` +${gained} ATK!` : ` No ATK gain this time.`} Enemy is stunned for 1 turn.` };
    }
  },
...
,
  ...
  ,
  {
    ...
    img: "cards/LeiRality.png",
    price: 25000,
    desc: "Damage: 18 • Armor: 22 • Life: 32 • Ability: Reality Requiem (HP ≤ 30%) — 50% +20 ATK, 100% stun enemy 1 turn (CD 3). Passive: Shield Hero — immune to instant death/one-hit kill; defense penetration damage -50%; missing HP becomes bonus DEF; on kill, remaining HP doubles.",
    playable: true
  }
...
function applyDamage(defender, dmg, opts = {}) {
  ...
  // 🛡️ LeiRality Passive: Shield Hero
  ...
  if (defender && defender.id === "leiRality" && dmg > 0) {
    ...
    if (ohko) {
      if (!opts.silent) {
        log(`🛡️ Shield Hero: ${defender.name} ignores instant death.`, "good");
    ...
    if (pen) {
      ...
      if (!opts.silent && before !== dmg) {
        log(`🛡️ Shield Hero: defense penetration reduced by 50% (${before} → ${dmg}).`, "good");
      }
    }
  }
...
function checkWin() {
  ...
  if (e.hp <= 0) {
    ...
    // 🛡️ LeiRality Passive: Shield Hero — on kill, remaining HP doubles
    if (p && p.id === "leiRality" && p.hp > 0) {
      ...
      p.hp = doubled;
      log(`🛡️ Shield Hero: ${p.name} doubles remaining HP (${beforeHp} → ${doubled}).`, "good");
...
const PATCH_NOTES = [

  {
    ...
    highlights: [
      ...
      "LeiRality passive: Shield Hero — immune to instant death/one-hit kill flags; defense penetration damage reduced by 50%; missing HP converts into bonus DEF; each kill doubles remaining HP.",
