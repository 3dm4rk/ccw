...
const BASE_CARDS = [
  ...
  },
  {
    ...
    skillDesc: "Attack twice. Each hit ignores 1 Armor. If below 50% HP, gain Dodge (1 turn: first enemy attack misses). (cooldown 2)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      const belowHalf = (Number(me.hp || 0) / Math.max(1, Number(me.maxHp || me.hp || 1))) < 0.5;
      if (belowHalf) me.dodge = 1;
...

{
  ...
  img: "cards/eyjies.png",
  price: 1000,
  desc: "Damage: 5 (TRUE) • Armor: 5 • Life: 12 • Passive: Every 2 turns, enemy ability cooldown +2; gain +1 Armor on trigger.",
  playable: true
},
...
function isBeginnerMode() {
  return !!(state && state.settings && state.settings.beginnerMode);
}
...
function pvpUseSkill(attacker, defender, attackerSide) {
  ...
  if (attacker.stun2Rounds > 0) {
    log(`${attacker.name} is stunned and cannot use a skill!`, "warn");
    pvpEnd(attackerSide, { auto:true, reason:"stunned" });
    return;
  }
...
const dmg = isEyjies ? 5 : (isHalakaReality ? dmgCalcNoRng(e) : dmgCalc(e));
...
// Ey-Ji-Es keeps its special: always 5 TRUE on basic attacks.
// ✅ Halaka Reality Form: basic attacks become TRUE damage equal to Halaka's CURRENT damage.
const dmg = isEyjies ? 5 : (isHalakaReality ? dmgCalcNoRng(p) : dmgCalc(p));
const dmgType = isEyjies ? "true" : (isHalakaReality ? "true" : (zukiTrue ? "true" : "physical"));
log(`${prefixMsg}${p.name} attacks for ${dmg} damage!`, "good");
...
// =========================
  ...
  // ✅ Centralized recovery: prevents soft-lock if an ability throws mid-turn (e.g., Zukinimato transformation edge cases)
  function battleRecoverFromError(err) {
    try {
      // Only recover during battle; don't interfere with menus.
      if (state && state.phase === "battle") {
        state.turn = "player";
        // Flush any modal action lock, if present
      ...
      // last resort: try to re-enable buttons directly
      try {
...
function closeAllLoreTooltips(e) {
  ...
  try {
    const bt = document.getElementById("beginnerToggle");
    if (bt) {
      bt.checked = isBeginnerMode();
      bt.addEventListener("change", () => {
        if (!state.settings) state.settings = { beginnerMode: true };
        state.settings.beginnerMode = !!bt.checked;
        saveSettings();
        try { updateUI(); } catch (e) {}
      });
    }
