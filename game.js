...
//
// Mission 6: 50 total wins -> unlocks Cosmo Secret (owned)
...
function isMission6Complete() {
  const wins = Math.max(0, Number(state?.profileWins ?? state?.profile?.wins ?? 0) || 0);
  return wins >= 50;
}
...
function updateMissionText() {
  ...
  // OLD Missions 1–6 are now Missions 6–11
    ...
    mainLine = `Mission 6: Win 50 battles to reveal the Revelation. (Wins: ${Math.min(totalWins, 50)}/50)`;
    subLine = totalWins >= 50 ? "Status: Complete ✅ — Cosmo Secret is unlocked." : "Status: Win battles until you reach 50 total wins.";
...
openModal({
    ...
    setTimeout(() => {
      ...
      // ✅ Mission 6: total 50 wins → unlock Cosmo Secret (once)
      {
        const totalWins = Math.max(0, Number(state.profileWins || 0) || 0);
        if (totalWins >= 50 && !(state.owned && state.owned["cosmoSecret"])) {
          unlockSecretStreakCard();
          log("🎯 Mission 6 complete: 50 total wins!", "good");
        }
      }
