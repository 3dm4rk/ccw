...
function healUnit(target, amount, opts = {}) {
  ...
  applyLeiRalityMissingHpDef(target);
...
// =========================
function applyLeiRalityMissingHpDef(unit, force = false) {
  if (!unit || unit.id !== "leiRality") return;
  const orig = Math.max(1, Number(unit.originalMaxHp || unit.maxHp || unit.hp || 1) || 1);
  unit.originalMaxHp = orig;

  if (unit._leiBaseShieldCap == null) {
    unit._leiBaseShieldCap = Math.max(0, Number(unit.shieldCap ?? unit.shield ?? unit.def ?? 0) || 0);
  }

  const curHp = Math.max(0, Number(unit.hp || 0) || 0);
  const missing = Math.max(0, Math.round(orig - curHp));
  const prev = Math.max(0, Number(unit._leiMissingHpDefApplied || 0) || 0);

  const delta = missing - prev;
  if (!force && delta === 0) return;

  unit._leiMissingHpDefApplied = missing;

  // Cap grows with missing HP so DEF can scale
  unit.shieldCap = Math.max(0, Number(unit._leiBaseShieldCap || 0) + missing);

  // Current shield (DEF) tracks the delta so it rises/falls smoothly
  unit.shield = Math.max(0, Number(unit.shield || 0) + delta);
  unit.shield = Math.min(unit.shieldCap, unit.shield);

  // Keep displayed DEF in sync with Shield
  unit.def = unit.shield;
}

function maybeInitLeiRality(unit) {
  if (!unit || unit.id !== "leiRality") return;
  if (unit.originalMaxHp == null) unit.originalMaxHp = Math.max(1, Number(unit.maxHp || unit.hp || 1) || 1);
  if (unit._leiBaseShieldCap == null) unit._leiBaseShieldCap = Math.max(0, Number(unit.shieldCap ?? unit.shield ?? unit.def ?? 0) || 0);
  if (unit._leiMissingHpDefApplied == null) unit._leiMissingHpDefApplied = 0;
  applyLeiRalityMissingHpDef(unit, true);
}

// =========================
// 🩸 LeiRality Skill HP Gate
// Skill usable ONLY when HP <= 40%
// =========================
function canUseLeiRalitySkill(unit) {
  // Non-Lei cards are unaffected
  if (!unit || unit.id !== "leiRality") return true;

  const maxHp = Math.max(1, Number(unit.maxHp || unit.originalMaxHp || unit.hp || 1) || 1);
  const curHp = Math.max(0, Number(unit.hp || 0) || 0);
...
function applyVoidTurnStartDamage(unit) {
  ...
  // 🌌 VOID COLLAPSE III baseline: fixed TRUE damage every turn
  const perTurn = Math.max(0, Number(vc.truePerTurn || 0) || 0);
  if (perTurn > 0) {
    applyDamage(unit, perTurn, { source: "void", damageType: "true", attackerName: "Void Collapse" });
    try { floatingDamage(side, `-${perTurn}`, "warn"); } catch (e) {}
    log(`💀 Void Collapse hits ${unit.name} for ${perTurn} TRUE damage!`, unit === state.player ? "bad" : "good");
  }

  // 🕳️ After VOID COLLAPSE III (Round 26+): "Void Execution"
  // Deal TRUE damage equal to 50% of the target's CURRENT HP (ignores Armor).
  // Note: rounds increment at the start of the player's turn later in nextTurn(),
  // so we treat player-turn-start as the upcoming round (round+1).
  const effectiveRound = (state.turn === "player") ? (Number(state.round || 1) + 1) : (Number(state.round || 1) || 1);
  if ((Number(vc.level || 0) >= 3) && effectiveRound >= 26 && Number(unit.hp || 0) > 0) {
    const hpNow = Math.max(0, Number(unit.hp || 0) || 0);
    let execDmg = Math.floor(hpNow * 0.5);
    if (execDmg < 1) execDmg = 1;

    applyDamage(unit, execDmg, { source: "void", damageType: "true", attackerName: "Void Execution" });
    try { floatingDamage(side, `☠️ -${execDmg}`, "warn"); } catch (e) {}
    log(`☠️ Void Execution (Round ${effectiveRound}+): ${unit.name} takes ${execDmg} TRUE damage (50% current HP).`, unit === state.player ? "bad" : "good");
  }
}
...
,
  ...
  leiRality: {
    ...
    def: 22,
    hp: 32,
    skillName: "Reality Requiem",
    skillDesc: "Activation: Only usable at 40% HP or lower. Gains +20 ATK (added to current). Stuns the enemy for 2 turns. (cooldown 3)",
    skill: (me, foe) => {
      
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };

      // Ensure originalMaxHp is set for the 40% threshold
      maybeInitLeiRality(me);
      const orig = Math.max(1, Number(me.originalMaxHp || me.maxHp || 1) || 1);
      const hpPct = (Number(me.hp || 0) || 0) / orig;

      if (hpPct > 0.40) {
        return { ok: false, msg: `Reality Requiem can only be used when HP is 40% or lower.` };
      }

      // Always gain +20 ATK
      me.atk = Math.max(0, Number(me.atk || 0) + 20);

      // Stun enemy for 2 turns (refresh/extend to at least 2)
      if (foe) foe.stunned = Math.max(2, Number(foe.stunned || 0) || 0);

      // keep cooldown behavior consistent with (cooldown 3) display: 4 means next 3 turns are blocked
      me.cooldown = 4;

      return { ok: true, msg: `${me.name} uses Reality Requiem! +20 ATK! Enemy is stunned for 2 turns.` };
    }
  },
...
eyJiEs: {
  ...
  stakesStaker: {
    ...
    hp: 25,
    skillName: "Dice Roll",
    skillDesc: "50% chance: convert current DEF into ATK permanently (stackable). On success, gain +4 DEF (stackable). (cooldown 2 turns)",
    skill: (me, foe) => {
      if (me.cooldown > 0) return { ok: false, msg: `Skill is on cooldown (${me.cooldown} turns).` };
      ...
      // Convert CURRENT DEF (shield) into ATK permanently (stackable)
      const curDef = Math.max(0, Number(me.def || me.shield || 0) || 0);
      const gainedAtk = curDef;

      if (gainedAtk > 0) {
        me.atk = Math.max(0, Number(me.atk || 0) || 0) + gainedAtk;

        // Swapped out of defense → remove current armor/def
        me.shield = 0;
        me.def = 0;
      }

      // Also gain +4 DEF permanently (stackable)
      const gainedDef = 4;
      const cap0 = Math.max(0, Number(me.shieldCap != null ? me.shieldCap : 0) || 0);
      me.shieldCap = cap0 + gainedDef;
      me.shield = Math.min(me.shieldCap, (Number(me.shield || 0) || 0) + gainedDef);
      me.def = me.shield;

      // Clear any legacy temporary fields (old Dice Roll behavior)
      me.diceRollTurns = 0;
      me._diceRollStoredDef = 0;

      me.cooldown = 3; // cooldown 2 turns
      return { ok: true, msg: `🎲 Dice Roll SUCCESS! +${gainedAtk} ATK (from DEF) and +${gainedDef} DEF (stackable).` };
    }
  },
...
//
// Mission 6: Win 50 battles in a row (fresh streak) after Mission 5 -> unlocks Cosmo Secret (owned)
...
function isMission6Complete() {
  // Mission 6 is a fresh 50-win streak earned AFTER Mission 5.
  // If the Cosmo card is already owned, treat as complete.
  if (state?.owned && state.owned["cosmoSecret"]) return true;

  if (!isMission5Complete()) return false;
  const s = Math.max(0, Number(state?.missions?.m6WinStreak || 0) || 0);
  return s >= 50;
}
...
function ensureMissionUnlockDefaults() {
  ...
  // ✅ Backfill Mission 7 completion for older saves:
  // If Ray Bill is already owned, the player must have defeated Cosmo Secret previously.
  if (!m.cosmoRevelationDefeated && state?.owned && state.owned["rayBill"]) {
    m.cosmoRevelationDefeated = true;
  }
...
,
  ...
  ,
  {
    ...
    img: "cards/LeiRality.png",
    price: 25000,
    desc: "Damage: 18 • Armor: 22 • Life: 32 • Ability: Reality Requiem (HP ≤ 30%) — 50% +20 ATK, 100% stun enemy 1 turn (CD 3). Passive: Defense Specialist — immune to instant death/one-hit kill; defense penetration damage -50%; missing HP becomes bonus DEF; on kill, remaining HP doubles.",
    playable: true
  }
...
function buyRelic(id) {
  ...
  // 🔒 Sequential gate: only allow Mission 4 completion after Missions 1–3 are completed.
  try {
    ensureMissionUnlockDefaults();
    if (
      isMission1Complete() &&
...
function redeemCodeApply(rawInput) {
  ...
  const REDEEM_CODES = {
    ...
    // 💰 Free Gold
    "FREEMONEY": { type: "gold", amount: 500000 },
// ✅ Alias codes (requested)
  ...
  if (reward.type === "gold") {
    const amt = Math.max(0, Math.floor(Number(reward.amount || reward.gold || 0) || 0));
    if (amt <= 0) throw new Error("This code reward is not configured correctly.");

    state.gold = Math.max(0, Math.floor(Number(state.gold || 0) || 0)) + amt;

    redeemed[norm] = { type: "gold", amount: amt, at: Date.now() };
    storageSet(REDEEMED_CODES_KEY, JSON.stringify(redeemed));
    saveProgress();

    // Refresh gold UI wherever the user is
    if (typeof updateGoldUI === "function") updateGoldUI();
    if (typeof renderShopCards === "function") renderShopCards();

    // Simple reward message (works even if the fancy card modal isn't used)
    try {
      if (typeof showArenaAlert === "function") showArenaAlert(`💰 +${amt.toLocaleString()} GOLD!`, "good");
      else alert(`Redeemed ✅\n\n+${amt.toLocaleString()} Gold`);
    } catch (e) {
      alert(`Redeemed ✅\n\n+${amt.toLocaleString()} Gold`);
    }
    return;
...
function unlockSecretStreakCard(force) {
  const isForced = !!force;

  // Safety gate: Cosmo should only be awarded after Mission 6 is actually completed
  // (unless explicitly forced by dev/admin paths).
  if (!isForced && !isMission6Complete()) return false;
  ...
  // Prevent repeat reveal popups
  if (!state.missions) state.missions = {};
  const alreadyShown = !!state.missions.cosmoRevealModalShown;
  if (!alreadyShown) state.missions.cosmoRevealModalShown = true;
  ...
  // 🌌 Cinematic reveal modal + lore (one-time)
  if (!alreadyShown) showCosmoRevealModal();
  return true;
}
...
function applyWinStreakMilestones() {
  ...
  // 50 streak: gold bonus only (Cosmo unlock is handled by Mission 6 now)
  if (s > 0 && s % 50 === 0) {
    bonus += 250;
    log(`🎁 50 streak reward → +250 gold.`, "good");
  }
...
function updateMissionText() {
  ...
  const totalWins = Math.max(0, Number(state?.profileWins ?? state?.profile?.wins ?? 0) || 0);
  const m6Streak = Math.max(0, Number(state?.missions?.m6WinStreak || 0) || 0);
  ...
  // OLD Missions 1–6 are now Missions 6–11
    ...
    mainLine = `Mission 6: Win 50 battles in a row to unlock Cosmo. (Streak: ${Math.min(m6Streak, 50)}/50)`;
    subLine = isMission6Complete() ? "Status: Complete ✅ — Cosmo Secret is unlocked." : "Status: Win 50 straight battles (a loss resets the streak).";
...
function cloneCard(card) {
  ...
  const unit = {
    ...
    // ✅ Debuff state (fix: ATK 0 no longer permanent; ATK-0 lasts 3 turns then restores)
    zeroAtkTurns: 0,
    _zeroAtkRestore: null,
    _debuffSnapSet: false,
    _debuffOrigAtk: null,
    _debuffOrigShield: null,
    _debuffAtkFrozen: false,
    stun2Rounds: 0,
...
// =========================
// 🧊 BATTLE FLOW LOCK (anti-freeze + anti-double-tap)
// - Prevents overlapping inputs / stacked timers that can soft-lock the game.
// - nextTurn() will release the lock automatically when it becomes the player's turn again.
// =========================
function setBattleBusy(on) {
  try {
    if (!state) return;
    state._busy = !!on;
    // Keep buttons responsive, but never allow actions while busy.
    updateUI();
  } catch (e) {}
}
function isBattleBusy() {
  try { return !!(state && state.phase === "battle" && state._busy); } catch (e) { return false; }
}
// =========================
...
try { setBattleBusy(false); } catch (e) {}
try { if (state && state._enemyTimer) { clearTimeout(state._enemyTimer); state._enemyTimer = null; } } catch (e) {}
  try {
...
function pvpApplyPotionEffectTo(p, e, potion){
  ...
  if (eff === "godsUltimate") {
    ...
    // Reward: small heal/armor (prevents huge Max HP spikes)
    const reward = Math.max(0, Math.ceil((Number(dmg || 0) || 0) * 0.10)); // 8–50
    p.hp = Math.min(Math.max(1, Number(p.maxHp || 1) || 1), (Number(p.hp || 0) || 0) + reward);
    p.shield = Math.min(getShieldCap(p), (Number(p.shield || 0) || 0) + reward);
    p.def = p.shield;
...
function floatingDamage(target, text, cls, opts = {}) {
  // Queue popups per-side so they never spam/overlap and never block taps.
  const side = (target === "enemy") ? "enemy" : "player";
  const msg = { text: String(text ?? ""), cls: String(cls || ""), dur: Math.max(250, Number(opts.dur || 900) || 900) };

  if (!window.__FLOAT_Q) {
    window.__FLOAT_Q = { player: [], enemy: [] };
    window.__FLOAT_ACTIVE = { player: false, enemy: false };
  }

  // Optional: clear existing queued messages (used at the start of a new action)
  if (opts && opts.clearFirst) {
    floatingDamageClear(side);
  }

  window.__FLOAT_Q[side].push(msg);
  _pumpFloatingDamage(side);
}

function floatingDamageClear(target) {
  const side = (target === "enemy") ? "enemy" : "player";
  try {
    if (window.__FLOAT_Q && window.__FLOAT_Q[side]) window.__FLOAT_Q[side].length = 0;
    // Remove existing DOM nodes immediately
    const frame = side === "player" ? $("pFrame") : $("eFrame");
    if (frame) {
      frame.querySelectorAll(".floatingDmg").forEach(el => { try { el.remove(); } catch (e) {} });
    }
    if (window.__FLOAT_ACTIVE && window.__FLOAT_ACTIVE[side]) window.__FLOAT_ACTIVE[side] = false;
  } catch (e) {}
}

function _pumpFloatingDamage(side) {
  try {
    if (!window.__FLOAT_Q || !window.__FLOAT_ACTIVE) return;
    if (window.__FLOAT_ACTIVE[side]) return;

    const frame = side === "player" ? $("pFrame") : $("eFrame");
    if (!frame) { window.__FLOAT_Q[side].length = 0; return; }

    const next = window.__FLOAT_Q[side].shift();
    if (!next) return;

    window.__FLOAT_ACTIVE[side] = true;

    const el = document.createElement("div");
    el.className = `floatingDmg ${next.cls || ""}`;
    el.textContent = next.text;
    frame.appendChild(el);

    setTimeout(() => {
      try { el.remove(); } catch (e) {}
      window.__FLOAT_ACTIVE[side] = false;
      // Small gap so messages feel readable
      setTimeout(() => _pumpFloatingDamage(side), 90);
    }, next.dur);
  } catch (e) {
    try { window.__FLOAT_ACTIVE[side] = false; } catch (e2) {}
  }
}
...
function applyDamage(defender, dmg, opts = {}) {
  ...
  // 🛡️ LeiRality Passive: Defense Specialist
  // If the incoming hit ignores defense (TRUE damage / penetration), cut the incoming damage by 50%.
  // Also block instant death / one-hit-kill flags (if any future skill uses them).
  if (defender && defender.id === "leiRality" && dmg > 0) {
    const pen = (damageType === "true") || !!opts.ignoreArmor || !!opts.defPenetration || !!opts.defensePenetration || (String(opts.damageType || "").toLowerCase() === "true");
    const ohko = !!opts.oneHitKill || !!opts.instantDeath || (String(opts.damageType || "").toLowerCase() === "execute");

    if (ohko) {
      if (!opts.silent) {
        log(`🛡️ Defense Specialist: ${defender.name} ignores instant death.`, "good");
    ...
    if (pen) {
      ...
      if (!opts.silent && before !== dmg) {
        log(`🛡️ Defense Specialist: defense penetration reduced by 50% (${before} → ${dmg}).`, "good");
      }
    }
  }
...
// so if a save file still has these timers, just clear them safely without reverting stats.
  ...
  if (f.debuffRounds > 0) {
    ...
    // Snapshot "normal" stats once, so we can restore when the debuff ends.
    // This prevents the common bug where ATK hits 0 and stays 0 forever after debuff expires.
    if (!f._debuffSnapSet) {
      f._debuffSnapSet = true;
      f._debuffOrigAtk = Number(f.atk || 0);
      f._debuffOrigShield = Number(f.shield || 0);
      f._debuffAtkFrozen = false;
    }

    // --- ATK side (special rule: if ATK becomes 0, keep it 0 for 3 turns, then restore) ---
    if (Number(f.zeroAtkTurns || 0) > 0) {
      // While disarmed, force ATK to 0.
      f.atk = 0;
      f.zeroAtkTurns = Math.max(0, Number(f.zeroAtkTurns || 0) - 1);

      if (f.zeroAtkTurns <= 0) {
        const restore = Number(
          (f._zeroAtkRestore != null ? f._zeroAtkRestore : (f._debuffOrigAtk != null ? f._debuffOrigAtk : 0))
        );
        f.atk = Math.max(0, restore);

        // Stop further ATK reductions for the remaining debuff duration,
        // otherwise it would instantly drop back to 0 again.
        f.debuffAtk = 0;
        f._debuffAtkFrozen = false;
        f._zeroAtkRestore = null;

        log(`⚔️ ${f.name}'s ATK recovered!`, "good");
      }
    } else if (dAtk > 0 && !f._debuffAtkFrozen) {
      const beforeAtk = Number(f.atk || 0);
      const afterAtk = Math.max(0, beforeAtk - dAtk);
      f.atk = afterAtk;

      if (afterAtk <= 0 && beforeAtk > 0) {
        // Enter 3-turn "ATK 0" lock, then restore.
        f.zeroAtkTurns = 3;
        f._zeroAtkRestore = (f._debuffOrigAtk != null) ? Number(f._debuffOrigAtk) : beforeAtk;
        f._debuffAtkFrozen = true;
        f.atk = 0;
        log(`🚫 ${f.name}'s ATK dropped to 0! ATK will recover in 3 turns.`, "bad");
      } else {
        log(`📉 ${f.name} suffers debuff (-${dAtk} ATK)!`, "warn");
      }
    }

    // --- Armor side (still ticks each round while debuff is active) ---
    if (dSh > 0) {
      f.shield = Math.max(0, Number(f.shield || 0) - dSh);
      // Keep DEF and Shield in sync
      f.def = f.shield;
      log(`🛡️ ${f.name} suffers debuff (-${dSh} Armor)!`, "warn");
    }

    // Consume a debuff round
    f.debuffRounds -= 1;

    // When debuff ends, restore stats back to the snapshot.
    if (f.debuffRounds <= 0) {
      // Restore ATK (unless currently in the 3-turn ATK-0 lock; that will restore itself)
      if (!(Number(f.zeroAtkTurns || 0) > 0) && f._debuffOrigAtk != null) {
        f.atk = Math.max(0, Number(f._debuffOrigAtk));
      }

      // Restore Armor back to snapshot (but never exceed current shield cap)
      if (f._debuffOrigShield != null) {
        const cap = getShieldCap(f);
        f.shield = Math.min(cap, Math.max(0, Number(f._debuffOrigShield)));
        f.def = f.shield;
      }
      ...
      // cleanup
      f._debuffSnapSet = false;
      f._debuffOrigAtk = null;
      f._debuffOrigShield = null;
      f._debuffAtkFrozen = false;
    }
  }
}
...
function spawnNextEnemy() {
  // ✅ SAFETY: always release any previous turn-lock so the new stage never spawns with disabled buttons
  setBattleBusy(false);
...
function _updateUIImmediate() {
  ...
  $("btnAttack").disabled = !playerTurn || busy;
// Skill button: disable while cooldown is running (turn-based OR real-time) and while Silenced.
  ...
  if (bs) {
    ...
    // 🔒 LeiRality: skill can only be used at 40% HP or lower
    const hpGateOk = (p.id !== "leiRality") || canUseLeiRalitySkill(p);
  ...
  $("btnEnd").disabled = !playerTurn || busy;
  ...
  if (bp) {
    ...
    bp.disabled = !playerTurn || busy || !hasAny;
...
function checkWin() {
  ...
  if (p.hp <= 0) {
    ...
    // Mission 6 streak breaks on any loss (fresh 50-win streak must be consecutive)
    if (!state.missions) state.missions = {};
    if (isMission5Complete() && !(state.owned && state.owned["cosmoSecret"]) && state.missions.m6Started) {
      state.missions.m6WinStreak = 0;
      try { saveProgress(); } catch (e) {}
    }
  ...
  if (e.hp <= 0) {
    ...
    // 🛡️ LeiRality Passive: Defense Specialist — on kill, remaining HP doubles
    if (p && p.id === "leiRality" && p.hp > 0) {
      const beforeHp = Number(p.hp || 0) || 0;
      const doubled = Math.floor(beforeHp * 2);
      p.maxHp = Math.max(Number(p.maxHp || 0) || 0, doubled);
      p.hp = doubled;
      log(`🛡️ Defense Specialist: ${p.name} doubles remaining HP (${beforeHp} → ${doubled}).`, "good");
      applyLeiRalityMissingHpDef(p);
    }
...
openModal({
    ...
    setTimeout(() => {
      ...
      // 🔒 Sequential gate: Missions 1–5 must be completed in order.
      // Prevent Mission 5 from completing early (e.g., duel win before finishing Missions 2–4).
      if (
        state.duelMode &&
        !state.missions.m5DuelWin &&
        isMission1Complete() &&
        isMission2Complete() &&
        isMission3Complete() &&
        isMission4Complete()
        ...
        // Start Mission 6 fresh streak tracking (must be NEW consecutive wins after Mission 5)
        if (!state.missions.m6Started) {
          state.missions.m6Started = true;
          state.missions.m6WinStreak = 0;
        }
      ...
      // 🌩️ Secret omen: defeating Cosmo Secret reveals Ray Bill + completes Mission 7
      if (e && e.id === "cosmoSecret") {
        // ✅ Mission 7 progress
        if (state.missions) state.missions.cosmoRevelationDefeated = true;

        // One-time omen modal (Ray Bill reveal)
        if (!state.rayBillOmenShown) {
          state.rayBillOmenShown = true;
          try { saveProgress(); } catch (e) {}
      ...
      // ✅ Mission 6: Win 50 battles in a row (fresh streak) AFTER Mission 5 → unlock Cosmo Secret (once)
      {
        if (!state.missions) state.missions = {};

        // Track Mission 6 streak only once the player has completed Mission 5.
        if (isMission5Complete() && !(state.owned && state.owned["cosmoSecret"])) {
          if (!state.missions.m6Started) {
            state.missions.m6Started = true;
            state.missions.m6WinStreak = 0;
          }

          state.missions.m6WinStreak = Math.max(0, Number(state.missions.m6WinStreak || 0) || 0) + 1;

          if (Number(state.missions.m6WinStreak || 0) >= 50) {
            const unlocked = unlockSecretStreakCard();
            if (unlocked) log("🎯 Mission 6 complete: 50-win streak!", "good");
          }
        }
      }
      ...
      // 🔒 Sequential gate: must finish Mission 1 first.
      if (
        isMission1Complete() &&
        !state.missions.m2ReachedStage5 &&
        Number(state.stage || 0) >= 5 &&
        !state.storyBossFight
      ) {
...
function nextTurn() {
  ...
  if (isPvpActive()) { setBattleBusy(false); updateUI(); return; }
...
if (state.turn === "player") tickStatuses(state.player);
  ...
  // ✅ Release action lock when it becomes the player's turn.
  if (state.turn === "player") {
    setBattleBusy(false);
  }
  ...
  if (state.turn === "player") {
    ...
    if (!isPvpActive() && state.turn === "enemy") {
      try { if (state._enemyTimer) clearTimeout(state._enemyTimer); } catch (e) {}
      state._enemyTimer = setTimeout(() => {
        try { enemyAI(); } finally { /* nextTurn will release lock when player turn starts */ }
      }, 420);
    }
...
function enemyAI() {
  if (state.phase !== "battle") { setBattleBusy(false); return; }
  // Lock stays on during enemy turn to prevent player taps queuing actions.
  setBattleBusy(true);
  try {
  try { floatingDamageClear("player"); floatingDamageClear("enemy"); } catch (e) {}
...
applyDamage(e, dmg, { source: "attack", damageType: dmgType, attacker: p, attackerName: p.name });
  ...
  if (!checkWin()) nextTurn();
    ...
    try { console.error(e); } catch (err) {}
    try { battleRecoverFromError(e); } catch (err2) {}
    // release lock so the player can continue
    setBattleBusy(false);
  }
}
...
function playerSkill() {
  ...
  if (isSilenced(p)) { log(`${p.name} is Silenced and cannot use a skill!`, "warn"); setBattleBusy(false); return; }
  const res = p.base.skill(p, e, state);
  if (!res.ok) { log(res.msg, "warn"); setBattleBusy(false); return; }
...
function applyPotionEffect(potion) {
  ...
  if (eff === "godsUltimate") {
    ...
    if (typeof log === "function") {
      log(`🧪 God's Ultimate activated! Enemy silenced (2 rounds), armor shattered, took ${dmg} TRUE damage. You gained +${reward} HP/+${reward} DEF.`, "good");
    }
  }
...
function openPotionUseModal(availablePotions) {
  ...
  (availablePotions || []).forEach((p) => {
    ...
    btn.addEventListener("click", () => {
      ...
      // 🎯 Mission 3: Use any potion in battle
      // 🔒 Sequential gate: only allow Mission 3 completion after Missions 1–2 are completed.
      try {
        ensureMissionUnlockDefaults();
        if (isMission1Complete() && isMission2Complete() && !state.missions.m3UsedPotion) {
...
function resetAll() {
  closeModal();
  // ✅ SAFETY: clear any battle lock (prevents unclickable buttons after restart/home)
  setBattleBusy(false);
    resetCardVisuals();
  // Reset special modes
  state.duelMode = false;
...
// 🔒 LeiRality: skill only usable at 40% HP or lower
if (p && p.id === "leiRality" && typeof canUseLeiRalitySkill === "function" && !canUseLeiRalitySkill(p)) {
  try { log("🩸 LeiRality can only use this ability at 40% HP or lower.", "warn"); } catch (e) {}
  try { floatingDamage("player", "🩸 NEEDS ≤40% HP", "warn"); } catch (e) {}
  return;
}
...
{
  version: "debuff-recovery-hotfix",
  date: "2026-01-27",
  highlights: [
    "Fixed debuff bug where ATK could drop to 0 forever after debuffs (ex: Dr. Nemesis).",
    "If a debuff drives ATK to 0, it now stays at 0 for 3 turns, then recovers to normal damage."
  ]
},


{
  version: "void-execution-v15",
  date: "2026-01-27",
  highlights: [
    "Void Collapse upgrade: starting Round 26+, a new 'Void Execution' triggers at turn start.",
    "Void Execution deals TRUE damage equal to 50% of the target’s current HP (ignores Armor).",
    "Void Collapse III’s baseline 8 TRUE damage per turn still applies."
  ]
},


{
  version: "battle-stability-v14",
  date: "2026-01-27",
  highlights: [
    "LeiRality skill button is now locked until HP is 40% or lower (UI + hard cast check).",
    "Improved tooltip/message so players understand why the skill is locked."
  ]
},

{
  version: "battle-stability-v13",
  date: "2026-01-27",
  highlights: [
    "LeiRality skill updated: Reality Requiem is now usable at 40% HP or lower; it always grants +20 ATK and stuns the enemy for 2 turns.",
    "LeiRality passive renamed to Defense Specialist (effects unchanged: immune to instant death/one-hit kill; defense penetration damage reduced by 50%; missing HP converts into bonus DEF; each kill doubles remaining HP).",
    "What’s New badge updated for the latest version."
  ]
},

{
  version: "battle-stability-v12",
  date: "2026-01-27",
  highlights: [
    "Added new redeem code: FREEMONEY (gives 500,000 gold).",
    "Redeem rewards now support non-card rewards (gold).",
    "What’s New badge updated for the latest version."
  ]
},

  {
    version: "battle-stability-v11",
    date: "2026-01-27",
    highlights: [
      "Added new shop card: LeiRality (25,000 gold).",
      "LeiRality skill: Reality Requiem — usable only at 30% HP or lower; 50% chance to gain +20 ATK; always stuns the enemy for 1 turn.",
      "LeiRality passive: Defense Specialist — immune to instant death/one-hit kill flags; defense penetration damage reduced by 50%; missing HP converts into bonus DEF; each kill doubles remaining HP.",
      "What’s New badge updated for the latest version."
    ]
  },

  {
    version: "battle-stability-v10",
    date: "2026-01-27",
    highlights: [
  ...
  {
    version: "battle-flow-cleanup-v9",
    date: "Jan 27, 2026",
    highlights: [
    ...
    changes: [
      ...
      "Enemy AI scheduling is now deduped (no stacked timeouts).",
    ...
    notes: [
      "If you still feel lag on very low-end phones, turn on your browser’s 'Reduce Motion' setting for extra smoothness."
    ]
  },
